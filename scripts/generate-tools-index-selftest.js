...
function tool(id, status, digest, promotionPath) {
  return {
    ...
    selftest: {
      ...
      sha256: digest,
      digestScope: SELFTEST_DIGEST_SCOPE
    }
  };
}
...
check('default options preserve the full non-verifying route', () => {
  assert.deepEqual(Generator.parseOptions([]), { verify: false, asOf: null, refreshClock: false, selectedToolIds: [], workers: 2, timeoutMs: 45000 });
});
...
check('explicit index clock controls survive scoped option parsing', () => {
  const options = Generator.parseOptions(['--verify', '--tool=alpha', '--as-of=2026-08-16T00:00:00Z', '--refresh-clock']);
  assert.equal(options.asOf, '2026-08-16T00:00:00Z');
  assert.equal(options.refreshClock, true);
});
...
const existingReceipt = {
  ...
  results: [
    { id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' },
    { id: 'beta', selftestSha256: 'sha-beta', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' },
    { id: 'delta', selftestSha256: 'stale-delta', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' },
    { id: 'gamma', selftestSha256: 'sha-gamma', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }
  ]
};
...
check('selected receipt merge replaces selected rows and retains only digest-current eligible rows', () => {
  const receipt = Generator.buildVerificationReceipt({
    preliminary,
    existingReceipt,
    latestResults: [{ id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'FAIL' }],
...
check('selected receipt merge requires exactly one latest row per selected id', () => {
  ...
  assert.throws(() => Generator.buildVerificationReceipt({ preliminary, existingReceipt, latestResults: [{ id: 'beta', selftestSha256: 'sha-beta', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }], selectedToolIds: ['alpha'], workers: 1, timeoutMs: 5000 }), /did not produce/);
});

check('latest rows must be eligible, digest-current and use a supported verdict', () => {
  assert.throws(() => Generator.buildVerificationReceipt({ preliminary, latestResults: [{ id: 'gamma', selftestSha256: 'sha-gamma', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }], selectedToolIds: ['gamma'], workers: 1, timeoutMs: 5000 }), /eligible target/);
  assert.throws(() => Generator.buildVerificationReceipt({ preliminary, latestResults: [{ id: 'alpha', selftestSha256: 'stale', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }], selectedToolIds: ['alpha'], workers: 1, timeoutMs: 5000 }), /digest does not match/);
  assert.throws(() => Generator.buildVerificationReceipt({ preliminary, latestResults: [{ id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: 'top-level-only/v0', verdict: 'PASS' }], selectedToolIds: ['alpha'], workers: 1, timeoutMs: 5000 }), /digest scope does not match/);
  assert.throws(() => Generator.buildVerificationReceipt({ preliminary, latestResults: [{ id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'UNKNOWN' }], selectedToolIds: ['alpha'], workers: 1, timeoutMs: 5000 }), /unsupported verdict/);
});
...
check('full receipts never retain unrelated historical rows', () => {
  const receipt = Generator.buildVerificationReceipt({
    ...
    latestResults: [
      { id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' },
      { id: 'beta', selftestSha256: 'sha-beta', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' },
      { id: 'delta', selftestSha256: 'sha-delta', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }
    ],
    selectedToolIds: [],
    workers: 2,
    timeoutMs: 45000,
...
check('full receipt construction rejects missing eligible results', () => {
  assert.throws(() => Generator.buildVerificationReceipt({
    preliminary,
    latestResults: [{ id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' }],
    selectedToolIds: [],
    workers: 2,
    timeoutMs: 45000
...
check('a valid checked-in index can seed digest-bound fallback evidence', () => {
  const result = { id: 'alpha', selftestSha256: 'sha-alpha', selftestDigestScope: SELFTEST_DIGEST_SCOPE, verdict: 'PASS' };
  const receipt = Generator.receiptFromIndex({
    ...
    promotionQueue: {},
    capabilities: [],
    tools: [{ id: 'alpha', status: 'TEST', selftest: { sha256: 'sha-alpha', digestScope: SELFTEST_DIGEST_SCOPE, result }, promotion: { state: 'BLOCKED' } }],
    truth: {
