...
function parseOptions(argv) {
  ...
  return {
    verify,
    asOf: asOfArg ? asOfArg.slice('--as-of='.length) : null,
    refreshClock: args.includes('--refresh-clock'),
    selectedToolIds,
...
function indexClock(options) {
  const declaredAsOf = options.asOf || process.env.AXM_INDEX_AS_OF || null;
  if (options.refreshClock && declaredAsOf) {
    throw new Error('--refresh-clock cannot be combined with --as-of or AXM_INDEX_AS_OF');
  }
  if (options.asOf) return validClock(options.asOf, '--as-of');
  if (process.env.AXM_INDEX_AS_OF) return validClock(process.env.AXM_INDEX_AS_OF, 'AXM_INDEX_AS_OF');
  if (!options.refreshClock) {
    try {
...
function failureTail(value, limit) {
  return DiagnosticRedaction.failureTail(value, { workspaceRoot:ROOT, limit });
}
...
function buildVerificationReceipt(options) {
  ...
  latestResults.forEach(row => {
    ...
    if (row.selftestDigestScope !== tool.selftest.digestScope) throw new Error('latest self-test result digest scope does not match current target: ' + row.id);
  ...
  if (selectedToolIds.length) {
    ...
    if (existing) {
      retained = validateReceipt(existing, 'existing self-test receipt').results.filter(row => {
        const tool = eligibleById.get(row.id);
        return !!tool && !selected.has(row.id) && row.selftestSha256 === tool.selftest.sha256 && row.selftestDigestScope === tool.selftest.digestScope;
      });
    }
  } else {
