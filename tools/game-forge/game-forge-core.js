(function(root,factory){var api=factory();if(typeof module!=='undefined'&&module.exports)module.exports=api;if(root)root.AXMGameForgeCore=api;})(typeof self!=='undefined'?self:this,function(){
  ...
  var VIEWS=[
    ...
    {id:'knowledge',title:'Game Production Atlas',short:'Production Atlas',group:'Forge',route:'knowledge',icon:'knowledge',description:'Search 500 game-production patterns and attach a balanced evidence-aware build plan to the active project.'},
  ...
  function blankProject(id,name,mode){var cells=[];for(var i=0;i<160;i++)cells.push({terrain:'empty',height:0});return{schema:'axm.game-forge-project/v1',id:id,name:name,version:'0.1.0',runtimeMode:mode||'2D',status:'DRAFT',createdAt:new Date().toISOString(),updatedAt:null,capabilityPlan:null,world:{width:16,height:10,cells:cells},physics:{schema:'axm.game-physics-config/v1',engine:'axm-physics-2d',engineVersion:'0.3.1',enabled:false,world:null,updatedAt:null},events:{nodes:[],edges:[]},systems:[],behaviors:[],tests:[],mods:[]};}
  return{VERSION:'1.1.0',VIEWS:VIEWS,TERRAIN:TERRAIN,EVENT_TYPES:EVENT_TYPES,SYSTEM_TYPES:SYSTEM_TYPES,BEHAVIOR_TYPES:BEHAVIOR_TYPES,byId:byId,groups:groups,blankProject:blankProject};
});
