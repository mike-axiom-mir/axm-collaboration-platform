...
async function startOrigin(port) {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('Relaybound origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('Relaybound origin did not become ready: ' + stderr.trim());
}
...
function createFaultProxy(origin) {
  ...
  const server = http.createServer((request, response) => {
    ...
    const tunnel = { upstream, upstreamResponse: null, response };
    active.add(tunnel);
  ...
  return { close, drop, isOffline: () => offline, listen, restore, server };
}

async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 21440 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start Relaybound origin');
