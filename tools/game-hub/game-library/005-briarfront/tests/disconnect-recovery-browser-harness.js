...
const serverFile = path.resolve(__dirname, '..', 'runtime', 'briarfront-server.cjs');
const wait = ms => new Promise(resolve => setTimeout(resolve, ms));

function evidenceRoster() {
  return ['Mike', 'Errol', 'Nova', 'Ivan'].map((display_name, index) => ({
...
async function startOrigin(port) {
  const child = spawn(process.execPath, [serverFile], {
    env: {
      ...
      AXM_SESSION_ID: 'briarfront-disconnect-evidence-session'
    },
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('Briarfront origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('Briarfront origin did not become ready: ' + stderr.trim());
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 22440 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start Briarfront origin');
