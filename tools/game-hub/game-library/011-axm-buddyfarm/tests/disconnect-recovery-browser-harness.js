...
async function startOrigin(port) {
  const child = spawn(process.execPath, [serverFile], {
    env: {
      ...
      AXM_GAME_SESSION_ID: 'buddyfarm-disconnect-evidence-session'
    },
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('BuddyFarm origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('BuddyFarm origin did not become ready: ' + stderr.trim());
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 25040 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start BuddyFarm origin');
