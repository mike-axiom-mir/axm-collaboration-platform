...
(async function main() {
  assert.equal(Runtime.selectedMode({AXM_GAME_PLAY_MODE: 'tycoon'}), 'tycoon');
  assert.equal(Runtime.selectedMode({AXM_GAME_PLAY_MODE: 'walkable-globe'}), 'walkable-globe');
  assert.equal(Runtime.modeEntry('tycoon'), '/games/010/game/tycoon-steward/');
  ...
  const server = Runtime.createServer({env: {AXM_GAME_PLAY_MODE: 'tycoon'}});
  await new Promise((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve); });
  try {
    const port = server.address().port;
    const health = await request(port, '/health');
    assert.equal(health.status, 200);
    assert.equal(JSON.parse(health.body).mode, 'tycoon');
    const state = JSON.parse((await request(port, '/api/launcher-state')).body);
    assert.equal(state.partyScreenLinks.all, '/games/010/game/tycoon-steward/');
