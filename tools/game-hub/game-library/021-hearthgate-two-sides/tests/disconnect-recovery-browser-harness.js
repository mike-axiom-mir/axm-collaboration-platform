...
const serverFile = path.resolve(__dirname, '..', 'runtime', 'server.js');
...
async function startOrigin(port) {
  const child = spawn(process.execPath, [serverFile], {
    env: {
      ...
      AXM_GAME_SESSION_ID: 'hearthgate-disconnect-evidence-session'
    },
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('Hearthgate origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('Hearthgate origin did not become ready: ' + stderr.trim());
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 24240 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start Hearthgate origin');
