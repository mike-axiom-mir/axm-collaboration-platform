...
## Department of Later growth pass - 2026-08-16

Release: `0.20.0-department-of-later` (**TEST / playable local alpha; unsealed**).

- Converted only Office of Temporal Mischief's ordinary Watchmoon into the Department of Later while preserving its `C` key, 50 Glow, 90 Scrap, 3.4-second construction, 390 durability, huge-vision role, and no-attack contract.
- Added an automatic fourteen-second pulse that deterministically chooses the nearest currently visible enemy formation within 520 and defers it for three seconds. Movement and attack recovery pause; its route, target, and macro order remain intact for resumption.
- Mixed nearby charters lengthen the deferral through the existing capped x1.45 wonderweb to 4.35 seconds. Buildings, allies, dead, distant, invalid, and already-deferred formations are excluded, so overlapping offices do not stack one filing onto the same active target.
- Preserved Wonderwork-first rival expansion, followed by ordinary Departments for Temporal. The Office of Already Done retains its separate recruitment and Essence-cooldown acceleration role.
- Added a temporal clock facade, `LATER` fascia, idle filing brackets, active source docket ring, target beam, cyan `FILED: LATER` shield, source duration label, and battle-feed receipt without a worker, manual ability, individual fighter control, routine siege, or co-op protocol change.

Verification: five runtime syntax checks **PASS**; focused district suite **20/20 PASS**; full Node suite **72/72 PASS**; package contract **69/69 PASS**; isolated HTTP suite **4/4 PASS**. Fresh health and launcher responses report v0.20, eight conversions, exact 14/3/4.35/520 boundaries, mixed-charter duration scaling, non-stacking exclusions, order preservation, building exclusion, and rival parity. An initial valid Tuesday run placed the office away from the rival route and ended in defeat at 03:34 without any trigger; it is counterevidence against claiming that a completed building alone proves behavior. A second 1280x720 Mild Tuesday run placed the Department on the home approach and produced a repeated same-viewport baseline-active-settled sequence: the active frame showed `DEFERRED 3.0s`, the source ring and beam, and a cyan `FILED: LATER` enemy-formation shield; 4.5 seconds later those temporary cues cleared while both the Department and formation remained. Browser warning/error logs were empty. No screenshot files were retained because the user did not request evidence artifacts; exact sub-frame animation cadence remains **UNKNOWN** because no rolling buffer was exposed. This pass is not sealed, promoted, or CANON.
