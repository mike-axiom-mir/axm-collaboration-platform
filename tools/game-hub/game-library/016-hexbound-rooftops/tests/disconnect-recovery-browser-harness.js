...
async function startOrigin(port) {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('Hexbound origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('Hexbound origin did not become ready: ' + stderr.trim());
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 24640 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start Hexbound origin');
...
module.exports = { createEvidenceHarness, createFaultProxy };
