...
const serverFile = path.resolve(__dirname, '..', 'runtime', 'lumenwake-server.cjs');
...
async function startOrigin(port) {
  const child = spawn(process.execPath, [serverFile], {
    env: {
      ...process.env,
      AXM_LUMEN_HOST: '127.0.0.1',
      PORT: String(port),
      AXM_PLAYERS_JSON: JSON.stringify(evidenceRoster()),
      AXM_SESSION_ID: 'lumenwake-disconnect-evidence-session'
    },
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    if (child.exitCode != null) throw new Error('Lumenwake origin exited early: ' + stderr.trim());
    try {
  ...
  child.kill('SIGTERM');
  throw new Error('Lumenwake origin did not become ready: ' + stderr.trim());
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 26040 + Math.floor(Math.random() * 1000);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start Lumenwake origin');
