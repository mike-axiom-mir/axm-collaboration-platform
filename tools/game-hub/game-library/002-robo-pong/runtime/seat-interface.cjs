...
const OUTCOME_FIELDS = new Set(['position', 'x', 'y', 'score', 'scores', 'winner', 'outcome', 'mission', 'ball', 'paddle', 'state']);
const INPUT_PROTOCOL = 'axm-semantic-input-v1';
const OBSERVATION_PROTOCOL = 'axm-seat-screen-semantics-v1';
const AUTHORITY_GATE = 'duet-seat-authority-v1';
...
function playerBySeat(room, seatId) {
  return Object.values(room.players).find(player => player.seatId === seatId) || null;
}
function createSeatBindings(room) {
  const bindings = {};
  for (const player of Object.values(room.players)) {
    if (!EXTERNAL_TYPES.has(player.kind) || !player.seatId) continue;
...
function buildAdapterObservation(room, request, context) {
  ...
  const publicPlayer = item => ({
    ...
    team: item.team,
    color: item.color,
    connected: item.controllerConnected
  });
  return {
    ok: true,
    schema: OBSERVATION_PROTOCOL,
    scope: 'seat-screen-visible-only',
    roomCode: state.room,
    ...
    arena: { id: arena.id, label: arena.label, chapter: arena.chapter, objective: arena.objective, width: state.width, height: state.height },
    self: { ...publicPlayer(state.players[player.id]), paddle: state.paddles[player.id], power: state.power[player.id] },
    hud: {
      scores: state.scores,
    ...
    limits: ['No seat tokens', 'No input buffers', 'No random generator state', 'No client-authored score, position, mission or outcome']
  };
}
