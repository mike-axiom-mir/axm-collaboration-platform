...
The house also contains the mapped mission board and, from v0.2.4, a separate collision-clear group-save computer. One controller selects a mission, the host draws one of four locations from that mission's v0.2.5 route deck, runs a 10-second countdown, then all actors teleport together. Replay draws a different location without removing the persistent results break. Mission success/failure returns the roster to the house; living damage is preserved and continues healing at 10 HP/s during results. After returning to `base` state, a connected human/adapter may use the save computer to write or load one of nine fixed-roster local group saves. Base upgrading, furnishing and equipment-driven regeneration remain unimplemented. District Dominion uses mirrored command posts instead of the cooperative house lifecycle.
