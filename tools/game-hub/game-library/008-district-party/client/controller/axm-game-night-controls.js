...
export function normalizeRadialInput(rawX, rawY, profile = DEFAULT_STICK_PROFILE) {
  ...
  const normalizedMagnitude = clamp((Math.min(1, rawMagnitude) - deadZone) / (1 - deadZone), 0, 1);
...
export class AxmVirtualStick {
  ...
  radius() {
    ...
    return Math.max(this.profile.minimumRadius, Math.min(bounds.width, bounds.height) * this.profile.radiusRatio);
  }
  ...
  setEnabled(enabled) {
    ...
    this.element.classList.toggle('secondary', !this.enabled);
