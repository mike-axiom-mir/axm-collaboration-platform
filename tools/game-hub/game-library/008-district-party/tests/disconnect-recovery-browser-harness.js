...
function evidenceRoster() {
  return [
    { seat_id: 'seat_1', slot: 1, display_name: 'Mike', type: 'human' },
    { seat_id: 'seat_2', slot: 2, display_name: 'Nova', type: 'human' }
  ];
}
...
async function startOrigin(port) {
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    if (child.exitCode != null) throw new Error('District Party origin exited early: ' + stderr.trim());
    try {
      const response = await fetch(origin + '/api/health');
      if (response.ok) {
        const launcher = await fetch(origin + '/api/launcher-state').then(value => value.json());
        const controller = launcher.controllerLinks.find(link => link.seatId === 'seat_2');
        if (!controller?.path) throw new Error('P2 controller link was not issued');
        return { child, controller, launcher, origin };
      }
    } catch (_) {}
    await wait(50);
  }
  child.kill('SIGTERM');
  throw new Error('District Party origin did not become ready: ' + stderr.trim());
}

async function stopOrigin(child) {
  if (!child || child.exitCode != null) return;
  child.kill('SIGTERM');
  await Promise.race([new Promise(resolve => child.once('exit', resolve)), wait(1800)]);
}
...
async function createEvidenceHarness(options = {}) {
  const preferredPort = Number(options.originPort) || 27040 + Math.floor(Math.random() * 900);
  ...
  if (!originRuntime) throw lastError || new Error('unable to start District Party origin');
  const proxy = createFaultProxy(originRuntime.origin);
  try {
    const address = await proxy.listen();
    let closed = false;
    return {
      controllerPath: originRuntime.controller.path,
      controller: originRuntime.controller,
      drop: proxy.drop,
      isOffline: proxy.isOffline,
      launcher: originRuntime.launcher,
...
async function runCli() {
  ...
  process.stdout.write(JSON.stringify({
    ...
    controllerPath: harness.controllerPath,
    sessionId: harness.launcher.sessionId,
    roomCode: harness.launcher.roomCode,
    seatId: harness.controller.seatId
  }) + '\n');
