...
The map is generated chunked JSON, not Tiled-authored. Its ground placement is derived from the official PDOK BGT OGC API under CC0 and then stylized onto the game grid; it is not a navigation map. No Google map tiles or labels were copied. Kenney's CC0 RPG Urban Pack remains the licensed fallback/small-prop family. v0.2.6 contains the first user-supplied AI-assisted AXM/Tilburg batch; v0.2.7 preserves the unpacked 158-PNG interactable source collection but loads only 14 inspected candidates. Neither user batch is labelled CC0. Mike authorized the project art for inclusion in the AXM public repository under `AXM-PROJECT-ART-PUBLIC-REPO-AUTHORIZATION-2026-07-19`; this is not an open-source or general standalone asset license. See `ASSET_PROVENANCE.md`, `assets/AXM_GENERATED_ART_AUTHORIZATION.md`, `assets/USER_GENERATED_ASSET_MANIFEST.json` and `assets/INTERACTABLE_ASSET_MANIFEST.json`.
