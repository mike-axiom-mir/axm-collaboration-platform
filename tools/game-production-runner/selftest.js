...
async function main() {
  ...
  equal(Cli.parse(['plan-cache-lease-tier-export', '--tier-root', 'T', '--minimum-tier-history-bytes', '6', '--maximum-tier-package-bytes', '7', '--maximum-tier-total-bytes', '8', '--max-tier-candidates', '9']).maximumTierTotalBytes, 8, 'CLI parses explicit tier root and package load budgets');
  equal(Cli.parse(['restore-cache-lease-tier-package', '--restore-cache-root', 'R', '--package-digest', 'a', '--approve-package', 'a', '--explicit-restore']).explicitRestore, true, 'CLI parses separate package approval and restore authority');
  ...
  try {
    ...
    const unrequestedCurationRoot = path.join(temporary, 'unrequested-curation-cache');
    const unrequestedCuration = invoke(['apply-cache-lease-curation', '--cache-root', unrequestedCurationRoot, '--approve-proposal', 'a'.repeat(64)]);
    equal(unrequestedCuration.status, 0, 'unrequested CLI lease curation is a safe no-op');
    ok(parsed(unrequestedCuration).status === 'NOT_REQUESTED' && !fs.existsSync(unrequestedCurationRoot), 'unrequested lease curation records refusal without creating a cache root');
    const boundedCurationPlan = invoke(['plan-cache-lease-curation', '--cache-root', cacheStore, '--minimum-released-age-ms', '0', '--minimum-expired-age-ms', '0', '--max-curation-candidates', '2']);
    ok(parsed(boundedCurationPlan).proposal.status === 'READY_WITH_LIMITS' && parsed(boundedCurationPlan).proposal.candidates.length === 2 && parsed(boundedCurationPlan).proposal.deferred_count === 1, 'CLI curation candidate budget defers excess eligible ledgers without hiding them');
    const curationPlan = invoke(['plan-cache-lease-curation', '--cache-root', cacheStore, '--minimum-released-age-ms', '0', '--minimum-expired-age-ms', '0', '--max-curation-candidates', '10']);
    equal(curationPlan.status, 0, 'CLI lease curation dry-run exits successfully');
    const curationPlanResult = parsed(curationPlan), curationProposal = curationPlanResult.proposal;
    ok(curationPlanResult.source_segment_deletion_performed === false && curationProposal.status === 'READY' && curationProposal.candidates.length === 3, 'CLI curation plan selects three exact released ledgers without deletion authority');
    const curationProposalFile = path.join(temporary, 'lease-curation-proposal.json');
    fs.writeFileSync(curationProposalFile, JSON.stringify(curationProposal, null, 2) + '\n');
    const wrongCurationApproval = invoke(['apply-cache-lease-curation', '--cache-root', cacheStore, '--proposal', curationProposalFile, '--approve-proposal', 'e'.repeat(64), '--explicit-apply']);
    equal(parsed(wrongCurationApproval).status, 'APPROVAL_MISMATCH', 'CLI wrong curation approval leaves every hot ledger intact');
    const appliedCuration = invoke(['apply-cache-lease-curation', '--cache-root', cacheStore, '--proposal', curationProposalFile, '--approve-proposal', curationProposal.digest, '--explicit-apply']);
    equal(appliedCuration.status, 0, 'CLI exact approved lease curation exits successfully');
    ok(parsed(appliedCuration).status === 'APPLIED' && parsed(appliedCuration).outcomes.length === 3 && parsed(appliedCuration).authority.source_segments_removed, 'CLI curation archives and removes exactly the three approved hot source segments');
    ...
    const cliTierRoot = path.join(temporary, 'cli-tier-store'), unrequestedTierRoot = path.join(temporary, 'unrequested-tier-store');
    const unrequestedTier = invoke(['apply-cache-lease-tier-export', '--cache-root', rollupCache, '--tier-root', unrequestedTierRoot, '--approve-proposal', 'a'.repeat(64)]);
    ok(unrequestedTier.status === 0 && parsed(unrequestedTier).status === 'NOT_REQUESTED' && !fs.existsSync(unrequestedTierRoot), 'unrequested CLI tier export is a sealed no-op and creates no tier root');
    const tierPlanCommand = invoke(['plan-cache-lease-tier-export', '--cache-root', rollupCache, '--tier-root', cliTierRoot, '--minimum-released-age-ms', '0', '--minimum-expired-age-ms', '0', '--minimum-tier-history-bytes', '1', '--maximum-tier-package-bytes', '268435456', '--maximum-tier-total-bytes', '536870912', '--max-tier-candidates', '10']);
    equal(tierPlanCommand.status, 0, 'CLI cold-history tier dry-run exits successfully');
    const tierPlanResult = parsed(tierPlanCommand), tierProposal = tierPlanResult.proposal, cliPackageDigest = tierProposal.candidates[0].package_digest;
    ok(tierPlanResult.tier_write_performed === false && tierPlanResult.source_history_deletion_performed === false && tierProposal.status === 'READY' && !fs.existsSync(cliTierRoot), 'CLI tier plan identifies the exact package without writing tier or source state');
    const tierProposalFile = path.join(temporary, 'tier-export-proposal.json');
    fs.writeFileSync(tierProposalFile, JSON.stringify(tierProposal, null, 2) + '\n');
    equal(parsed(invoke(['apply-cache-lease-tier-export', '--cache-root', rollupCache, '--tier-root', cliTierRoot, '--proposal', tierProposalFile, '--approve-proposal', 'b'.repeat(64), '--explicit-apply'])).status, 'APPROVAL_MISMATCH', 'CLI wrong tier proposal approval writes no package');
    const appliedTier = invoke(['apply-cache-lease-tier-export', '--cache-root', rollupCache, '--tier-root', cliTierRoot, '--proposal', tierProposalFile, '--approve-proposal', tierProposal.digest, '--explicit-apply']);
    ok(appliedTier.status === 0 && parsed(appliedTier).status === 'APPLIED' && parsed(appliedTier).source_files_removed === 0, 'CLI exact tier approval writes one package without source removal');
    const auditedTier = invoke(['audit-cache-lease-tier-package', '--tier-root', cliTierRoot, '--package-digest', cliPackageDigest]);
    ok(auditedTier.status === 0 && parsed(auditedTier).status === 'COMPLETE' && parsed(auditedTier).payload_file_count === 4, 'CLI package audit verifies the sealed anchor and rollup payload');
    const cliRestoreRoot = path.join(temporary, 'cli-tier-restore'), unrequestedRestoreRoot = path.join(temporary, 'unrequested-tier-restore');
    const unrequestedRestore = invoke(['restore-cache-lease-tier-package', '--tier-root', cliTierRoot, '--package-digest', cliPackageDigest, '--restore-cache-root', unrequestedRestoreRoot, '--approve-package', cliPackageDigest]);
    ok(unrequestedRestore.status === 0 && parsed(unrequestedRestore).status === 'NOT_REQUESTED' && !fs.existsSync(unrequestedRestoreRoot), 'unrequested CLI restore is a sealed no-op and creates no target');
    equal(parsed(invoke(['restore-cache-lease-tier-package', '--tier-root', cliTierRoot, '--package-digest', cliPackageDigest, '--restore-cache-root', cliRestoreRoot, '--approve-package', 'c'.repeat(64), '--explicit-restore'])).status, 'APPROVAL_MISMATCH', 'CLI wrong package approval writes no restore target');
    const restoredTier = invoke(['restore-cache-lease-tier-package', '--tier-root', cliTierRoot, '--package-digest', cliPackageDigest, '--restore-cache-root', cliRestoreRoot, '--approve-package', cliPackageDigest, '--explicit-restore']);
    ok(restoredTier.status === 0 && parsed(restoredTier).status === 'RESTORED' && parsed(restoredTier).full_audit_equivalence.source_archive_summary_digest === parsed(restoredTier).full_audit_equivalence.restored_archive_summary_digest, 'CLI explicit restore completes with exact full-audit equivalence');
    ok(parsed(invoke(['audit-cache-lease-archives', '--cache-root', rollupCache])).usage.history_events === 15, 'CLI tier export and restore leave source history unchanged');
