...
the same mechanics while keeping the adapter boundary visible.
...
verifiers that are not in the v0.1 fixture inventory. Both run commands use
inert declarative fixtures and cannot write inside the Workshop. The portable
demo proves reuse of orchestration mechanics, not a universal production
kernel or documentation quality.
