...
const checks=[
 ...
 ['Evidence Desk has a gated preview-then-place Knowledge Canvas handoff',()=>{const html=read('tools/evidence-desk/index.html'),app=read('tools/evidence-desk/app.js');return /id="previewKnowledge"/.test(html)&&/id="placeKnowledge"/.test(html)&&/function previewKnowledge\(\)/.test(app)&&/function placeKnowledge\(\)/.test(app)&&/gate\('knowledge-handoff\.preview'/.test(app)&&/gate\('knowledge-handoff\.place'/.test(app)&&/addEventListener\('click', previewKnowledge\)/.test(app)&&/addEventListener\('click', placeKnowledge\)/.test(app);}],
