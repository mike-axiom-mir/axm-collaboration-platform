(function () {
  ...
  var feedEnabled = false, feedKnown = false;
  ...
  function checkFeed() {
    ...
    fetch('/services/mirror-native/axm/v1/learning/action-feed/status', { cache: 'no-store' }).then(function(response){ if(!response.ok) throw Error('HTTP '+response.status); return response.json(); }).then(showFeed).catch(function(){ feedKnown=false; feedState.textContent='UNAVAILABLE'; feedState.className=''; feedDetail.textContent='Mirror runtime is not available yet. Rechecking does not change its preference.'; feedToggle.textContent='Check again'; feedToggle.disabled=false; });
  }
  function changeFeed() {
    feedToggle.disabled = true;
    fetch('/services/mirror-native/axm/v1/learning/action-feed/settings', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({ enabled:!feedEnabled, actor:{ actor_id:'mike', actor_kind:'human', display_name:'Mike' } }) }).then(function(response){ if(!response.ok) return response.json().then(function(body){ throw Error(body.error||('HTTP '+response.status)); }); return response.json(); }).then(showFeed).catch(function(error){ feedKnown=false; feedState.textContent='HELD'; feedState.className=''; feedDetail.textContent=error.message+' Rechecking is read-only.'; feedToggle.textContent='Check again'; feedToggle.disabled=false; });
  }
  function check() {
  ...
  feedToggle.onclick = function(){ if(feedKnown) changeFeed(); else checkFeed(); };
  var initialized = false;
  function init(){ if(initialized) return; initialized=true; check(); checkFeed(); }
  if (window.AXMHub) { AXMHub.onInit(init); AXMHub.ready({ id: 'mirror-learning-shell', name: 'AI Learning Forge', version: 'v0.3-action-lessons', hubApiVersion: '1.0', permissions: [], savesState: false, handlesShutdown: false }); init(); } else init();
}());
