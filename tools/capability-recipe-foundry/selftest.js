...
(async function main() {
  ...
  check(adapterInspection.ok&&adapterInspection.active===false&&!Fabric.ALLOWED_BUILDERS.includes(adapterProposal.recipe.builderId)&&Fabric.loadCatalog().recipes.some((recipe)=>recipe.id===adapterProposal.recipe.id&&recipe.version==='0.2.0'&&recipe.builderId==='closed-object-contract-adapter-v2'),'historical v1 proposal remains inactive while the separately hardened strict v2 recipe is active');
