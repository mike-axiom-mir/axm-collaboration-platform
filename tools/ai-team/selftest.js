...
assert.equal(Core.VIEWS.length,12);
...
['task','duo','agents','forge','prompts','models','reasoning','explore','data'].forEach(route=>assert.equal(Core.VIEWS.filter(v=>v.route===route).length,1));
...
assert.equal(manifest.id,'ai-team');assert.equal(contract.version,'v1.5');
assert.ok(contract.boundaries.refuses.includes('automatic-guardian-reset'));
console.log('AI Team selftest: PASS (12 views, recommendation-only Specialist Router, AI-native masks, capability bridges, evidence and Guardian)');
