...
function usage() {
  return [
    'AXM Deterministic PR Publisher',
    '',
    'Plan (remote reads only):',
    '  node pr-publisher-cli.js plan --repo <git-root> --publish-root <authorized-root> --checkpoint <checkpoint.json> --verification <verification.json> --review <review-packet.json> [--out <plan.json>]',
    'Verify plan:',
    '  node pr-publisher-cli.js verify-plan --packet <plan.json> [--repo <git-root>] [--out <verification.json>]',
    'Publish exact branch and create/update a draft PR:',
    '  node pr-publisher-cli.js publish --repo <git-root> --publish-root <authorized-root> --checkpoint <checkpoint.json> --verification <verification.json> --review <review-packet.json> --plan <plan.json> --confirmation "' + Core.CONFIRMATION + '" --out <receipt.json>',
    'Verify receipt:',
    '  node pr-publisher-cli.js verify-receipt --packet <receipt.json> [--repo <git-root>] [--out <verification.json>]',
    '',
    'Publish requires a host-authenticated GitHub CLI session. Credentials are never read or retained by AXM.',
    'This tool never stages, commits, force-pushes, merges, deletes a branch, promotes, changes roots, or changes CANON.'
  ].join('\n');
}
...
function parseArguments(argv) {
  ...
  const action = argv[0];
  if (!['plan', 'verify-plan', 'publish', 'verify-receipt'].includes(action)) throw new Error('action must be plan, verify-plan, publish, or verify-receipt');
  const values = { action };
...
function sourcePackets(options) {
  return {
    checkpoint:readJson(options.checkpoint, '--checkpoint'),
    verification:readJson(options.verification, '--verification'),
...
function main(argv, dependencies) {
  ...
  if (options.action === 'verify-plan') result = Core.verifyPlan(readJson(options.packet, '--packet'));
    ...
    const packets = sourcePackets(options);
    if (options.action === 'plan') {
      const facts = Host.inspectHost({ repositoryRoot:options.repo, publishRoot:options.publishRoot, checkpoint:packets.checkpoint, runner:dependencies && dependencies.runner });
      result = Core.buildPlan(Object.assign({}, packets, { facts }));
    } else {
      if (!options.out) throw new Error('publish requires --out so the transport receipt is preserved outside the repository');
      result = Host.publishExact(Object.assign({}, packets, {
  ...
  emit(result, options);
  return ['HELD', 'FAIL', 'REFUSED', 'REMOTE_BRANCH_PUSHED_PR_UNCONFIRMED'].includes(result.state) ? 2 : 0;
}
