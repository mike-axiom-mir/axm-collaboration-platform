...
/* AXM Hermes Runtime Bootstrap v0.4
   ...
   Hermes stays an external MIT runtime pinned to an immutable commit+tree.
   AXM owns the launch profile, consent/tool policy, provider-credential guard,
   per-run evidence capsule, Return Packet, and repair path.

   This remains an authorization/evidence layer, NOT an OS/network sandbox.
*/
'use strict';
...
function sourceLock() {
  ...
  if (!lock || lock.schema !== 'axm.hermes-source-lock/v1' || !lock.repo_url ||
      !/^[0-9a-f]{40}$/i.test(lock.commit || '') || !/^[0-9a-f]{40}$/i.test(lock.tree || '') ||
      !lock.observed_version) {
    throw new Error('Invalid hermes-source.lock.json; commit, tree, version and source are required.');
  }
  return lock;
}
...
function managedBlock(python) {
  ...
  const gate = hookCommand(python, GATE);
  const context = hookCommand(python, CONTEXT);
  const receipt = hookCommand(python, RECEIPT);
  const provider = hookCommand(python, PROVIDER_RECEIPT);
  const session = hookCommand(python, SESSION_EVENT);
  return [
    MANAGED_BEGIN,
    ...
    '      fail_closed: true',
    '  pre_llm_call:',
    '    - command: ' + scalar(context),
    '      timeout: 5',
    '  post_tool_call:',
    '    - command: ' + scalar(receipt),
    '      timeout: 5',
    '  pre_api_request:',
    '    - command: ' + scalar(provider),
    '      timeout: 5',
    '  post_api_request:',
    '    - command: ' + scalar(provider),
    '      timeout: 5',
...
function mergeManagedConfig(current, block, force, policy) {
  ...
  if (/^\s*hooks\s*:/m.test(current)) {
    ...
    return block + '\n' + current.replace(/^\s*hooks\s*:[\s\S]*$/m, '# Previous unmanaged hooks removed by explicit AXM repair --force.\n');
  }
...
function prepare(force) {
  ...
  const wanted = mergeManagedConfig(current, managedBlock(py), force, policy);
  if (current !== wanted) {
    if (current && policy.repair && policy.repair.backup_before_managed_config_change !== false) {
      console.log('Config backup: ' + backupFile(HERMES_CONFIG, 'hermes-config', policy.repair.retain_config_backups || 5));
    }
...
function verifyHermesHomeCredentialPolicy(policy) {
  ...
  if (policy.provider_egress.mode === 'local_only') {
    throw new Error('local_only provider posture refuses secret-bearing HERMES_HOME/.env; use process-local loopback credentials or explicit_remote policy. Keys: ' + keys.join(', '));
  }
...
function auditConsent(previous, next, reason) {
  ...
  fs.appendFileSync(path.join(AUDIT_DIR, 'consent-events.jsonl'), JSON.stringify(record) + '\n', 'utf8');
}
function setConsent(enabled, reason) {
  if (!exists(POLICY_FILE)) prepare(false);
  const policy = loadPolicy();
  const previous = policy.consent.enabled === true;
  ...
  auditConsent(previous, enabled, text);
...
function repairInterruptedRuns() {
  ...
  for (const runId of incompleteRuns()) {
    ...
    RunLedger.finalizeRun({
      runId, runDir,
      receiptDir: path.join(runDir, 'receipts'), providerReceiptDir: path.join(runDir, 'provider-receipts'), sessionEventDir: path.join(runDir, 'session-events'),
      reportDir: REPORT_DIR, sourceLock: lock, policyFile: POLICY_FILE, configFile: HERMES_CONFIG,
      sourceVerified: manifest.source && manifest.source.verified_before_launch === true, exitCode: null, signal: 'recovered-after-incomplete-run'
    });
    repaired += 1;
  }
  return repaired;
...
function repair(force) {
  prepare(force);
  const source = verifyPinnedSource(false);
  const policy = loadPolicy();
  const credentials = verifyHermesHomeCredentialPolicy(policy);
  const repaired = repairInterruptedRuns();
  ...
  console.log('  orphan runs:      ' + repaired + ' closed into recovery Return Packets');
}
...
function showDoctor() {
  ...
  const policy = readJson(POLICY_FILE);
  const policyCheck = RuntimePolicy.validatePolicy(policy);
...
function installPinned() {
  ...
  if (!exists(HERMES_DIR)) {
    const r = run('git', ['clone', '--no-checkout', lock.repo_url, HERMES_DIR]);
    if (r.status !== 0) throw new Error('Git clone failed with status ' + r.status);
  }
  const remote = gitValue(['remote', 'get-url', 'origin']);
  if (normalizeRemote(remote) !== normalizeRemote(lock.repo_url)) throw new Error('Existing Hermes origin differs from reviewed source lock. Refusing repoint.');
...
function startHermes(extraArgs) {
  ...
  const policy = loadPolicy();
  verifyHermesHomeCredentialPolicy(policy);
  const guarded = guardedEnvironment(policy);
  const lock = sourceLock();
  const runContext = RunLedger.createRun({
    ...
    configFile: HERMES_CONFIG, args: extraArgs, environmentReport: guarded.report
  });
  Object.assign(guarded.env, {
    AXM_HERMES_RUN_ID: runContext.runId,
    AXM_HERMES_RUN_DIR: runContext.runDir,
    AXM_HERMES_STATE_DIR: runContext.stateDir,
    AXM_HERMES_RECEIPT_DIR: runContext.receiptDir,
    AXM_HERMES_PROVIDER_RECEIPT_DIR: runContext.providerReceiptDir,
    AXM_HERMES_SESSION_EVENT_DIR: runContext.sessionEventDir
  });
  ...
  const result = run('uv', args, { cwd: WORKSPACE, env: guarded.env });
  const packet = RunLedger.finalizeRun({
    ...
    reportDir: REPORT_DIR, sourceLock: lock, policyFile: POLICY_FILE, configFile: HERMES_CONFIG,
    sourceVerified: true, exitCode: result.status, signal: result.signal || null
  });
  console.log('Return Packet: ' + path.join(runContext.runDir, 'return-packet.json'));
  console.log('Outcome:       ' + packet.outcome);
  if (packet.integrity_flags.provider_policy_mismatch) console.error('AXM WARNING: provider receipt observed remote base URL under local_only policy. Treat run as boundary failure.');
