...
function main(){
  ...
  check(Registry.activeIds().length===6&&Registry.reviewCandidateIds().join(',')==='closed-object-contract-adapter-v1','registry contains six reviewed active builders and one exact inactive adapter candidate');
  ...
  check(Fabric.canonicalJson(Fabric.loadCatalog())===Fabric.canonicalJson(catalogBefore)&&Registry.activeIds().length===6&&Registry.reviewCandidateIds().length===1,'admission replay and receipt tests perform no activation or catalog mutation');
