...
(async function(){
  ...
  const list=await Machine.run({action:'catalog.list'});assert(list.ok&&list.catalog.recipes.length===3&&list.generatedCodeExecuted===false&&list.providerCalled===false&&Object.values(list.authority).every(function(value){return value===false;}));
  ...
  process.stdout.write('Capability Fabric tool selftest PASS · 10 checks\n');
