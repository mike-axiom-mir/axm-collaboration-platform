...
(async function(){
  ...
  const list=await Machine.run({action:'catalog.list'});assert(list.ok&&list.catalog.recipes.length===4&&list.catalog.builderRegistry.entries.filter(function(row){return row.status==='REVIEW_CANDIDATE';}).length===1&&list.generatedCodeExecuted===false&&list.providerCalled===false&&Object.values(list.authority).every(function(value){return value===false;}));
