...
(async function(){
  ...
  const list=await Machine.run({action:'catalog.list'});const reviewCandidates=list.catalog.builderRegistry.entries.filter(function(row){return row.status==='REVIEW_CANDIDATE';});const pythonRecipe=list.catalog.recipes.find(function(row){return row.id==='bounded-python-record-transform';});const cssRecipe=list.catalog.recipes.find(function(row){return row.id==='bounded-css-token-stylesheet';});const adapterRecipe=list.catalog.recipes.find(function(row){return row.id==='closed-object-contract-adapter';});const queryRecipe=list.catalog.recipes.find(function(row){return row.id==='bounded-record-query';});const fsmRecipe=list.catalog.recipes.find(function(row){return row.id==='bounded-portable-fsm-definition';});assert(list.ok&&list.catalog.recipes.length===11&&pythonRecipe&&pythonRecipe.capabilityKind==='HAND'&&pythonRecipe.builderId==='bounded-python-record-transform-v1'&&cssRecipe&&cssRecipe.capabilityKind==='HAND'&&cssRecipe.builderId==='bounded-css-token-stylesheet-v1'&&adapterRecipe&&adapterRecipe.builderId==='closed-object-contract-adapter-v2'&&queryRecipe&&queryRecipe.builderId==='bounded-record-query-v1'&&fsmRecipe&&fsmRecipe.builderId==='bounded-portable-fsm-definition-v1'&&reviewCandidates.length===1&&reviewCandidates[0].id==='closed-object-contract-adapter-v1'&&list.generatedCodeExecuted===false&&list.providerCalled===false&&Object.values(list.authority).every(function(value){return value===false;}));
