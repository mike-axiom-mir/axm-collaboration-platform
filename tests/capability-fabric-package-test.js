...
function main(){
  ...
  try{
    ...
    check(new Set(packageDigests).size===10,'ten capability recipes produce distinct packages');
    const registry=Nursery.scanSupply(nurseryRoot);check(registry.summary.total===10&&registry.summary.readyForLaterIntake===10,'Nursery independently sees all ten exact candidates ready for later intake after disjoint trusted tests');
