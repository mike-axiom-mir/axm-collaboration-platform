...
function main(){
  ...
  try{
    ...
    check(new Set(packageDigests).size===11,'eleven capability recipes produce distinct packages');
    const registry=Nursery.scanSupply(nurseryRoot);check(registry.summary.total===11&&registry.summary.readyForLaterIntake===11,'Nursery independently sees all eleven exact candidates ready for later intake after disjoint trusted tests');
