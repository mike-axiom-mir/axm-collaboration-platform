...
async function main() {
  await checkedGet('workshop.new-modules', 'http://127.0.0.1:8788/api/tools', r => { const ids = (r.json && r.json.tools || []).map(x => x.id); return { pass: ids.includes('ai-task-talk') && ids.includes('workshop-packager'), detail: ['ai-task-talk','workshop-packager'].filter(id => ids.includes(id)).join(', ') || 'new modules missing' }; });
  ...
  await checkedGet('hub.ai-presence-ui', 'http://127.0.0.1:8788/hub/ai-presence.js', r => ({ pass: r.status === 200 && r.body.includes("BRIDGE+'/health'") && r.body.includes("BRIDGE+'/local-models'") && r.body.includes("BRIDGE+'/agents/pause'") && r.body.includes('axm-llama-3.1-8b') && r.body.includes('gemini-local'), detail: r.status + ' · connector truth, identity status, and master agent pause gate present' }));
