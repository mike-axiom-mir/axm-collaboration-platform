...
function main() {
  ...
  try {
    ...
    check(!Fabric.ALLOWED_BUILDERS.includes(adapterProposal.recipe.builderId)&&BuilderRegistry.describe(adapterProposal.recipe.builderId).status===BuilderRegistry.REVIEW_CANDIDATE&&Fabric.loadCatalog().recipes.some((recipe)=>recipe.id===adapterProposal.recipe.id&&recipe.version==='0.2.0'&&recipe.builderId==='closed-object-contract-adapter-v2'),'adapter materialization leaves v1 inactive and the separately hardened strict v2 recipe unchanged');
