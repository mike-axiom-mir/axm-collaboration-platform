...
const IMPLEMENTATION_CONTRACT = Object.freeze({
  version: 'candidate-targeted-prohibition-v2',
...
function discover(workshopRoot) {
  ...
  if (contracts.length > MAX_CONTRACTS) throw new Error(`contract curriculum holds: ${contracts.length} eligible contracts exceed the ${MAX_CONTRACTS} contract bound`);
...
function derive(options = {}) {
  ...
  const results = [];
  try {
    for (const record of discovery.contracts) {
      const basis = evaluatorBasis(record);
      ...
      const sessionName = `session-${digest({ moduleId: record.moduleId, boundaryId: record.boundaryId }).slice(0, 20)}.json`;
      const sessionRelative = path.join('sessions', sessionName).replace(/\\/g, '/');
      const bytes = Buffer.from(json(session), 'utf8');
      fs.writeFileSync(path.join(stageDir, sessionRelative), bytes, { flag: 'wx' });
      results.push({
        moduleId: record.moduleId,
    ...
    const batch = {
      ...
      boundary: 'Typed boundaries.refuses entries may generate bounded private exams. Generated wording is not authority, held-out modules never enter training, learned outputs cannot grade or train this organ, and no examined action is executed.'
    };
...
module.exports = {
  ORGAN_ID, SCHEMA, EVALUATION_SCHEMA, SUPERSEDED_ORGAN_ID, IMPLEMENTATION_CONTRACT, EVALUATOR_PREFIX, MAX_CONTRACTS, HELD_OUT_MODULUS,
  digest, partitionFor, chooseBoundary, discover, evaluatorBasis, examInput,
  verifyDeterministicSession, verifyBatch, verifyHeldOutEvaluation, derive, evaluateHeldOut
};
