...
function summarizeDimensions(snapshot) {
  return {
    ...
    directRegressions: snapshot.dimensions.filter(item => item.state === 'REGRESSION_REQUIRES_REVIEW').length,
...
function buildReceipt(request, observation) {
  ...
  const receipt = {
    ...
    state: dimensions.directRegressions || dimensions.longitudinalRegressions
...
function verifyReceipt(receipt, runDir) {
  ...
  if (!['OBSERVATION_RECORDED_WITHOUT_REGRESSION', 'OBSERVATION_RECORDED_WITH_REGRESSION_REQUIRES_REVIEW'].includes(receipt.state)) throw new Error('foundation observation execution state changed');
  if (runDir) {
