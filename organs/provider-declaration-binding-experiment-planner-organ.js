...
const PLANNER_CONTRACT = Object.freeze({
  version: 'implementation-evidence-to-balanced-provider-binding-experiment-matrix-v1',
...
function verifyBatch(batch, runDir, implementationDerived, workshopRoot) {
  ...
  if (!same(batch.summary, expectedSummary(batch.results))) throw new Error('provider binding experiment summary mismatch');
  return true;
}
...
function derive(options = {}) {
  ...
  const results = foundations.map(foundation => {
    ...
    const sourceDigest = foundation.experimentMatrix ? foundation.experimentMatrix.matrixDigest : digest(foundation.requirementId);
...
module.exports = {
  ...
  expectedSummary, verifyBatch, derive, respond
};
