...
async function runInteractive(capabilityId) {
  ...
  if (!capabilityId) throw new Error('a capability-id selector is required before any voluntary prompt');
  Builder.verifyRecorded();
  const exact = Route.selectExact(capabilityId);
  ...
  try {
    ...
    for (const trial of exact.packet.trials) {
      ...
      if (decision === 'WITHDRAW') {
        const receipt = withdrawalReceipt(exact.protocol, state, isoNow());
    ...
    const session = Human.buildSession(exact.protocol, {
      sessionId: state.sessionId,
      generatedAt: completedAt,
      fixtureMode: 'LIVE',
      startedAt: state.startedAt,
...
if (require.main === module) {
  runInteractive(process.argv[2]).catch((error) => {
    console.error(error.stack || error.message);
    process.exitCode = 1;
  });
}

module.exports = { runInteractive, withdrawalReceipt };
