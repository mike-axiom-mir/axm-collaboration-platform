...
const PRODUCT_COMMIT = '3b8730c522c7441eb7b17d10c4fed0e9ca2c39da';
const PRODUCT_TREE = '788be63cd49c73abbf3e7480e539d9769863e896';
const commands = [
...
function run(command, cwd) {
  ...
  const result = childProcess.spawnSync(executable === 'node' ? process.execPath : executable, parts, { cwd, encoding:'utf8', windowsHide:true, timeout:180000, maxBuffer:64 * 1024 * 1024 });
  const exitCode = Number.isInteger(result.status) ? result.status : 1;
  process.stdout.write((exitCode === 0 ? 'PASS ' : 'FAIL ') + command + '\n');
  return { command, exitCode, verdict:exitCode === 0 ? 'PASS' : 'FAIL', diagnostic:exitCode === 0 ? null : String(result.stderr || result.stdout || result.error || '').trim().slice(-2000) };
}
function git(args, cwd) { return childProcess.spawnSync('git', args, { cwd, encoding:'utf8', windowsHide:true, timeout:180000, maxBuffer:16 * 1024 * 1024 }); }
...
const container = fs.mkdtempSync(path.join(path.parse(ROOT).root, 'r43-'));
...
try {
  ...
  const extract = childProcess.spawnSync('tar', ['-xf',archiveFile,'-C',container], { cwd:ROOT, encoding:'utf8', windowsHide:true, timeout:180000, maxBuffer:16 * 1024 * 1024 });
  ...
  const receipt = {
    schema:'axm.clean-archived-product-slice-replay/v1', status:results.every(result => result.exitCode === 0) && changedTrackedFiles.length === 0 ? 'PASS' : 'FAIL',
    productCommit:PRODUCT_COMMIT, productTree:PRODUCT_TREE, slicePaths, productSliceArchiveSha256:'sha256:' + sha256(fs.readFileSync(archiveFile)), trackedFiles:trackedNames.length, commands:results,
    ...
    trackedFilesUnchangedAfterReplay:changedTrackedFiles.length === 0, changedTrackedFiles:changedTrackedFiles.slice(0, 20), changedTrackedFilesTruncated:changedTrackedFiles.length > 20,
    replayDigest:null
  };
  const digestBody = JSON.parse(JSON.stringify(receipt)); delete digestBody.replayDigest;
  receipt.replayDigest = 'sha256:' + crypto.createHash('sha256').update(JSON.stringify(digestBody)).digest('hex');
  ...
  if (!cleanupComplete) { console.error('clean replay worktree cleanup failed'); process.exitCode = 1; }
}
