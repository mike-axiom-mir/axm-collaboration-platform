...
const EVIDENCE_COMMIT = '5e8cad7503b18a558fc6cadd751b54dd0c992471';
const EVIDENCE_TREE = '459f6f3b8ea5bc001b74d31c4bdb2748d5d310a1';
const EVIDENCE_PATH = 'docs/steward-runs/2026-08-21-review-inbox-operation-lease';
...
const container = fs.mkdtempSync(path.join(path.parse(ROOT).root, 'e43-'));
...
try {
  ...
  const validJsonLines = lines.filter(line => { try { JSON.parse(line); return true; } catch (_) { return false; } }).length;
  const sealReplay = {
  ...
  if (!cleanupComplete) { console.error('clean evidence replay cleanup failed'); process.exitCode = 1; }
}
