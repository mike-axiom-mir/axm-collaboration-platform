...
const prior = JSON.parse(fs.readFileSync(path.join(ROOT, 'docs/steward-runs/2026-08-21-review-inbox-host-key-projection-recovery/CHECK_RESULTS.json'), 'utf8'));
...
const focused = [
  ...
  ...prior.commands.filter(item => item.phase === 'FOCUSED' && !replaced.has(item.command)).map(item => item.command)
];
const required = prior.commands.filter(item => item.phase === 'REQUIRED').map(item => item.command);

function args(command) {
  const parts = command.split(' ');
  if (parts.shift() !== 'node' || parts.length !== 1 || !/^[A-Za-z0-9._/-]+\.js$/.test(parts[0]) || parts[0].includes('..')) throw new Error('only one bounded relative Node.js check is supported');
  return parts;
}
function assertionCount(output) {
  return Array.from(String(output || '').matchAll(/(?:SUMMARY\s+|RESULT\s+)?(\d+)\s+(?:focused\s+)?(?:assertions?|checks?|evidence groups?|controls?)\b/gi)).reduce((sum, match) => sum + Number(match[1]), 0);
}
function execute(command, phase) {
  ...
  return { command, phase, assertions:phase === 'FOCUSED' ? assertionCount(result.stdout) : 0, exitCode, verdict:exitCode === 0 ? 'PASS' : 'FAIL', diagnostic };
}
...
const receipt = {
  schema:'axm.verification-check-results/v1', status:commands.every(item => item.exitCode === 0) ? 'PASS' : 'FAIL', checkedAt:new Date().toISOString(), commands,
  summary:{ commands:commands.length, passed:commands.filter(item => item.exitCode === 0).length, failed:commands.filter(item => item.exitCode !== 0).length, focusedAssertions:commands.reduce((sum, item) => sum + item.assertions, 0) },
  resultsDigest:null
};
const payload = JSON.parse(Core.canonicalJson(receipt)); delete payload.resultsDigest;
