...
check(receipt.schema === 'axm.grounded-growth-json-root-cohort-check-results/v1' && receipt.status === 'TEST', 'verification receipt identity matches');
...
check(receipt.summary.focusedPassed === receipt.summary.focusedTotal, 'every focused and adjacent runnable check passes');
...
check(receipt.summary.knownExpectedStale === 3 && receipt.knownExpectedStale.length === 3, 'three historical source-identity checks stay explicitly stale');
check(receipt.knownExpectedStale.every((item) => item.verdict === 'EXPECTED_STALE_AFTER_SOURCE_CHANGE'), 'stale checks are not relabeled as passes');
check(result.requirements.find((item) => item.id === 'clean-checkout-full-verification').verdict === 'NOT_RUN', 'cohort build record honestly predates the final matrix');
...
check(!receipt.limits.historicalSourceIdentityReceiptsRegenerated, 'historical dated evidence remains immutable');
...
check(seal.parseStatus === 'valid' && seal.eventLines === 8 && seal.invalidJsonLines === 0, 'eight-event segment is structurally sealed');
...
check(curation.status === 'SEALED' && curation.sealDigest === seal.sha256 && curation.durableEventsPreserved === 8, 'curation receipt binds the sealed history');
check(curation.knownExpectedStale === 3 && curation.unclassifiedItems.length === 0, 'curation preserves known stale evidence without ambiguity');
check(curation.privateOrUserSourcesRetained === false, 'curated evidence retains no private user source material');

console.log('PASS Grounded Growth JSON root cohort verification selftest (' + checks + ' assertions)');
