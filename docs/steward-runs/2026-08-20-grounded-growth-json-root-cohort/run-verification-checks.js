...
const FOCUSED = [
  ...
  'node docs/steward-runs/2026-08-19-verification-evolution-grounded-growth/build-verification-evolution-growth.js --check-recorded',
  'node docs/steward-runs/2026-08-19-grounded-growth-current-convergence/build-current-convergence.js --check-recorded',
...
function build() {
  ...
  const receipt = {
    schema: 'axm.grounded-growth-json-root-cohort-check-results/v1',
    version: '0.1.0',
    status: 'TEST',
    checkedAt: new Date().toISOString(),
    checks,
    knownExpectedStale: KNOWN_EXPECTED_STALE.map((command) => ({
      command: 'node ' + command,
      verdict: 'EXPECTED_STALE_AFTER_SOURCE_CHANGE',
      reason: 'historical dated evidence binds the previous source-file digest and is preserved unchanged'
    })),
    summary: {
      total: checks.length,
      ...
      knownExpectedStale: KNOWN_EXPECTED_STALE.length
    },
    retention: {
    ...
    limits: {
      ...
      allConsumerMigrationRun: false,
      historicalSourceIdentityReceiptsRegenerated: false
    },
    authority: {
      install: false,
...
module.exports = { ROOT, OUTPUT, FOCUSED, REQUIRED, KNOWN_EXPECTED_STALE, assertionCount, build };
