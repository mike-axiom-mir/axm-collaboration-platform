...
const contract = JSON.parse(fs.readFileSync(path.resolve(dir, '../../../shared/model-shadow-retention-audit-review-outcome/module.contract.json'), 'utf8'));
...
const capabilities = requirements.requirements.flatMap(requirement => requirement.capabilities).map(id => {
  if (inherited.has(id)) {
    return { id, status: 'available', constraints: inherited.get(id) };
  }
  if (provided.has(id)) {
    return {
      id,
      status: 'available',
      constraints: ['TEST data-only adapter; exact rebuild is required for actor-digest provenance; no Review Inbox mutation, host observation, provider, process, or network authority']
    };
  }
  return {
    id,
    status: 'unknown',
    constraints: ['not implemented or proven by this bounded v3.1 leaf']
  };
});
const output = {
