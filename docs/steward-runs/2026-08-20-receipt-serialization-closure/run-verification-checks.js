...
function build() {
  ...
  const receipt = {
    schema: 'axm.receipt-serialization-closure-check-results/v1',
    version: '0.1.0',
    status: 'TEST',
  ...
  receipt.resultsDigest = 'sha256:' + require('crypto').createHash('sha256').update(Core.canonicalJson(payload)).digest('hex');
  return receipt;
}
