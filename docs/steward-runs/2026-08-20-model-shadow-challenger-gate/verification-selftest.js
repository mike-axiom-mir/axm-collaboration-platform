...
check(events.some(event => event.event === 'refusal_boundary_added') && events.some(event => event.event === 'review_gate_added'), 'refusal and review-gate decisions remain durable');
check(events.some(event => event.event === 'privacy_boundary_added') && events.some(event => event.event === 'open_evidence_preserved'), 'privacy and open-evidence decisions remain durable');
...
check(index.openEvidence.length === 5 && index.openEvidence.includes('held-out human benefit'), 'index preserves five unrun real-world seams');
check(sourceSnapshot.sources.length === 9 && sourceSnapshot.sources.every(item => {
...
check(results.status === 'PASS' && results.summary.commands === 18 && results.summary.passed === 18, 'all recorded verification commands passed');
check(results.summary.focusedAssertions === 280, 'focused assertion count remains exact');

console.log('\nModel Shadow Challenger Gate verification evidence: PASS (' + checks + ' checks)');
