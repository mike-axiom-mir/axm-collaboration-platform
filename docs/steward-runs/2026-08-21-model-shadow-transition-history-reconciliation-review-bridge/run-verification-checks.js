...
const prior = JSON.parse(fs.readFileSync(path.join(ROOT, 'docs/steward-runs/2026-08-21-model-shadow-retention-audit-review-outcome-transition-settlement-history-pairwise-observer/CHECK_RESULTS.json'), 'utf8'));
const FOCUSED = [
...
function assertionCount(output) {
  return Array.from(String(output || '').matchAll(/(?:SUMMARY\s+|RESULT\s+)?(\d+)\s+(?:focused\s+)?(?:assertions?|checks?|evidence groups?)\b/gi)).reduce((sum, match) => sum + Number(match[1]), 0);
}
...
const receipt = {
  schema:'axm.verification-check-results/v1', status:commands.every(item => item.exitCode === 0) ? 'PASS' : 'FAIL',
  checkedAt:new Date().toISOString(), commands,
