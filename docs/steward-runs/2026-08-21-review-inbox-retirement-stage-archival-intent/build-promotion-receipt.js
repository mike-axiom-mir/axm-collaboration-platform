...
const notPass = source.results.filter(item => item.verdict !== 'PASS').map(item => ({
  id:item.id, verdict:item.verdict, exitCode:item.exitCode
}));
const receipt = {
  ...
  truth:{
    ...
    readyForHumanReviewIsPromotion:false,
    readyForHumanReviewIsCanon:false,
    otherToolFailuresConvertedToPass:false,
    failureTailsRetained:false,
