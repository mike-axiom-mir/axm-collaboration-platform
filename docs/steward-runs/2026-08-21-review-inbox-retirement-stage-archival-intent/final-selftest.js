...
check(String(primary.stdout || '').includes('166 checks'),'primary evidence selftest retains 166 checks');
const replay = json('CLEAN_EVIDENCE_REPLAY.json');
equal(replay.status,'PASS','clean evidence replay passes');
equal(replay.evidenceCommit,'2403673c41bea053a2ee45801d2926cfdbf043f6','replay binds primary evidence commit');
equal(replay.evidenceTree,'188df66f77ee1957558431edbab1ef1d3a266706','replay binds primary evidence tree');
equal(replay.trackedFiles,32,'replay archives thirty-two tracked evidence dependency files');
equal(replay.selftest.verdict,'PASS','archived primary selftest passes');
equal(replay.selftest.stdout,'PASS Review Inbox archival intent v5.2 evidence: 166 checks','archived selftest output is exact');
...
equal(events.map(item => item.sequence),[21,22],'addendum continues the session sequence');
...
equal(index.productCommit,'b707aa08f0adf958f6344babf0839c37ab1b9dca','final index binds product commit');
equal(index.productTree,'0f2aa8e9e2fad6e19416c93667cf426899955eed','final index binds product tree');
...
equal(index.branch,'codex/grounded-growth-retirement-stage-archival-intent-v5.2','final index binds review branch');
...
console.log('PASS final archival intent v5.2 evidence: ' + assertions + ' checks');
