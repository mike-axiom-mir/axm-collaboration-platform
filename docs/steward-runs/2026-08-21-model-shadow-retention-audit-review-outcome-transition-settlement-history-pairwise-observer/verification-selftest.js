...
check(events.length === 43, 'segment retains forty-three semantic events');
...
[
  ...
  'independent_schema_validator_checked_unavailable', 'complete_history_contract_gap_found', 'capability_gap_before_compared',
  'triple_inspect_double_capture_added', 'artifact_self_digest_recheck_added', 'complete_history_commitment_added',
  'minimization_test_false_positive_found', 'minimization_test_narrowed', 'undefined_export_test_harness_failure_found',
  'undefined_export_test_harness_fixed', 'focused_initial_green', 'post_green_exact_snapshot_blind_spot_found',
  'same_snapshot_counterexample_strengthened', 'focused_final_green', 'all_side_and_pairwise_classifications_verified',
  'same_snapshot_hidden_held_history_verified', 'fresh_process_and_read_only_behavior_verified',
  'transient_mutation_boundary_preserved', 'joint_pair_replacement_counterexample_preserved',
  'browser_verification_not_applicable', 'full_inherited_verification_passed', 'post_run_workspace_change_classified',
  'authority_custody_globality_and_benefit_boundaries_preserved', 'evidence_selftest_label_mismatch_found',
  'evidence_selftest_label_corrected', 'frontier_evidence_phrase_mismatch_found',
  'frontier_evidence_phrase_corrected', 'mike_merge_and_canon_gate_preserved'
...
check(receipt.telemetryAggregation.retainedRawLogs === false && receipt.telemetryAggregation.commandOutcomes === 57, 'curation retains outcomes without raw logs');
check(receipt.telemetryAggregation.failedCommands === 0 && receipt.telemetryAggregation.focusedAssertions === 5405, 'curation retains exact verification totals');
...
check(index.openEvidence.includes('actual external retention or independent custody of roots'), 'external custody remains open');
check(index.openEvidence.includes('atomic current complete-history snapshot transient-reversion exclusion or later currentness'), 'atomic current history remains open');
check(index.openEvidence.includes('withheld history exclusion global uniqueness or globally consistent log'), 'globality remains open');
...
check(sources.sources.length === 326 && sources.sources.every(item => { const normalized = fs.readFileSync(path.join(root, item.path), 'utf8').replace(/\r\n?/g, '\n'); return item.sha256 === 'sha256:' + crypto.createHash('sha256').update(normalized).digest('hex'); }), 'all normalized source digests still match');
...
check(results.summary.focusedAssertions === 5405, 'focused assertion count remains exact');
check(routes.routes.length === 5 && routes.routes.every(route => route.verdict === 'PASS'), 'all five evidence routes retain PASS verdicts');

console.log('\nSettlement history pairwise-observer verification evidence: PASS (' + checks + ' checks)');
