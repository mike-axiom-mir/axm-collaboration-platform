...
function spawn(command, args, cwd, timeout) {
  return childProcess.spawnSync(command, args, { cwd, encoding:'utf8', windowsHide:true, timeout:timeout || 180000, maxBuffer:64 * 1024 * 1024 });
}
...
const container = fs.mkdtempSync(path.join(os.tmpdir(), 'axm-v49-clean-product-'));
...
try {
  ...
  if (!resolved.startsWith(allowed)) throw new Error('temporary replay root escaped the OS temp directory');
  fs.rmSync(resolved, { recursive:true, force:true });
  retained = fs.existsSync(resolved);
}
const receipt = {
  schema:'axm.clean-product-replay/v1',
  status:results.every(item => item.exitCode === 0) && trackedFilesUnchangedAfterReplay && !retained ? 'PASS' : 'FAIL',
  productCommit:product, productTree, slicePaths, commands:results,
  trackedFiles,
  ...
  trackedFilesUnchangedAfterReplay, changedTrackedFiles,
  temporaryReplayPathRetained:retained,
  specialistPackageIntakeAttempted:false,
  replayDigest:null
};
