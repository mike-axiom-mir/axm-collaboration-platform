...
const EVIDENCE_COMMIT = 'f08f83309de2b80b8dda1a5e6aaea154cccb313f';
const EVIDENCE_TREE = '645851340ff5df3990fe52b0eeb471230a8458b3';
const EVIDENCE_PATH = 'docs/steward-runs/2026-08-21-review-inbox-retirement-intent-withdrawal';
...
const temp = fs.mkdtempSync(path.join(os.tmpdir(),'axm-v49-clean-evidence-')), archive = path.join(temp,'evidence.tar');
try {
  ...
  const receipt = {
    ...
    sealReplay, immutableGitObjectDatabaseUsedForProductBlobVerification:true,
    sourceCheckoutOrSharedMainMutated:false, temporaryReplayPathRetained:false,
  ...
  if (!resolved.startsWith(allowed)) throw new Error('temporary evidence root escaped the OS temp directory');
