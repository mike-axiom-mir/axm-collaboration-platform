...
const commands = [
  ...
  ['FOCUSED', 'node shared/operations/selftest.js'],
...
function run(phase, command) {
  ...
  const assertions = phase === 'FOCUSED' ? Array.from(output.matchAll(/(\d+)\s+(?:assertions?|checks?|controls?)\b/gi))
  ...
  return {
    ...
    diagnostic:exitCode === 0 ? null : String(result.stderr || result.stdout || result.error || '').trim().slice(-1600)
  };
}

const results = commands.map(item => run(item[0], item[1]));
const receipt = {
  schema:'axm.verification-check-results/v1', status:results.every(item => item.exitCode === 0) ? 'PASS' : 'FAIL',
  checkedAt:new Date().toISOString(), commands:results,
  summary:{
    commands:results.length, passed:results.filter(item => item.exitCode === 0).length,
