...
const EVIDENCE_PREFIX = 'docs/steward-runs/2026-08-21-review-inbox-operation-lease-retirement-recovery/';
...
const changed = String(git(['diff-tree', '--no-commit-id', '--name-only', '-r', parent, PRODUCT_COMMIT]))
  .split(/\r?\n/).filter(Boolean);
if (changed.some(file => /AXM_MIRROR_SHADOW_SPECIALIST/i.test(file))) throw new Error('excluded specialist package lane appeared in product commit');
...
const snapshot = {
  schema:'axm.normalized-source-snapshot/v1', status:'TEST', commit:PRODUCT_COMMIT, parent, tree,
