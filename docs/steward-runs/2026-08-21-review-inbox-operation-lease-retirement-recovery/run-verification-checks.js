...
const prior = JSON.parse(fs.readFileSync(path.join(
  ROOT,
  'docs/steward-runs/2026-08-21-review-inbox-operation-lease-retirement/CHECK_RESULTS.json'
), 'utf8'));
const focused = prior.commands.filter(item => item.phase === 'FOCUSED').map(item => item.command);
focused.unshift('node shared/operations/review-operation-lease-retirement-recovery-selftest.js');
