...
function args(command) { const parts = command.split(' '); if (parts.shift() !== 'node') throw new Error('only explicit node checks are supported'); return parts; }
function assertionCount(output) { return Array.from(String(output || '').matchAll(/(\d+)\s+(?:assertions?|checks?|evidence groups?)/gi)).reduce((sum, match) => sum + Number(match[1]), 0); }
function execute(command, phase) {
  const result = childProcess.spawnSync(process.execPath, args(command), { cwd: ROOT, encoding: 'utf8', windowsHide: true, timeout: 180000, env: { ...process.env, AXM_TEST_TEMP: os.tmpdir() } });
...
const receipt = {
  schema: 'axm.verification-check-results/v1', status: results.every(item => item.exitCode === 0) ? 'PASS' : 'FAIL', checkedAt: new Date().toISOString(), commands: results,
