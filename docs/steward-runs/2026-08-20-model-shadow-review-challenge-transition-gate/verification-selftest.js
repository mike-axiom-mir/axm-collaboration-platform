...
check(events.some(event => event.event === 'transition_gate_leaf_added'), 'transition gate leaf remains a durable event');
check(events.some(event => event.event === 'independently_valid_fork_counterevidence_preserved'), 'independently valid fork counterevidence remains durable');
check(events.some(event => event.event === 'typed_transition_contradictions_proven'), 'typed transition contradictions remain durable');
check(events.some(event => event.event === 'fresh_process_transition_reload_proven'), 'fresh-process transition reload remains durable');
...
check(index.openEvidence.length === 12 && index.openEvidence.includes('globally consistent transition log') && index.openEvidence.includes('learning'), 'index preserves twelve protected, authority, human and outcome seams');
check(sourceSnapshot.sources.length === 44 && sourceSnapshot.sources.every(item => {
...
check(results.status === 'PASS' && results.summary.commands === 25 && results.summary.passed === 25, 'all recorded verification commands passed');
check(results.summary.focusedAssertions === 857, 'focused assertion count remains exact');

console.log('\nModel Shadow review challenge transition gate verification evidence: PASS (' + checks + ' checks)');
