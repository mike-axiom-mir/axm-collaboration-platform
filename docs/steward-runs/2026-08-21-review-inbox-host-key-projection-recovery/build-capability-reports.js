...
for (const phase of ['BEFORE','AFTER']) {
  const output = path.join(__dirname, 'CAPABILITY_GAP_' + phase + '.json');
  const result = childProcess.spawnSync('python', [comparator, '--requirements', path.join(__dirname, 'CAPABILITY_REQUIREMENTS.json'), '--capabilities', path.join(__dirname, 'CAPABILITY_INVENTORY_' + phase + '.json'), '--output', output], { encoding:'utf8', windowsHide:true });
