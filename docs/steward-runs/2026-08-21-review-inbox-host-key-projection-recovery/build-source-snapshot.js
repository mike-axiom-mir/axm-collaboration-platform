...
const EVIDENCE_PREFIX = 'docs/steward-runs/2026-08-21-review-inbox-host-key-projection-recovery/';
...
const productFiles = changed.filter(file => !file.startsWith(EVIDENCE_PREFIX)).sort().map(file => {
...
if (!productFiles.length) throw new Error('source snapshot found no product files');
const snapshot = {
  ...
  files:productFiles, productDigest:null
};
...
console.log('PASS source snapshot ' + productFiles.length + ' product files ' + snapshot.productDigest);
