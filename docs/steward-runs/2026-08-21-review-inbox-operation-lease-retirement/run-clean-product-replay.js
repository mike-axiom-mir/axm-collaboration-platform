...
const container = fs.mkdtempSync(path.join(path.parse(ROOT).root, 'r44-'));
...
try {
  ...
  receipt = {
    ...
    slicePaths,
    productSliceArchiveSha256: 'sha256:' + sha256(fs.readFileSync(archiveFile)),
    ...
    changedTrackedFiles: changedTrackedFiles.slice(0, 20),
    changedTrackedFilesTruncated: changedTrackedFiles.length > 20,
  ...
  if (!cleanupComplete) {
    console.error('clean replay workspace cleanup failed');
    process.exitCode = 1;
  }
}
