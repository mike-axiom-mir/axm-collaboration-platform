...
const prior = JSON.parse(fs.readFileSync(path.join(
  ROOT,
  'docs/steward-runs/2026-08-21-review-inbox-operation-lease/CHECK_RESULTS.json'
), 'utf8'));

const focused = prior.commands
  .filter(item => item.phase === 'FOCUSED')
  .map(item => item.command);
focused.unshift('node shared/operations/review-operation-lease-retirement-selftest.js');
focused.push('node scripts/package-script-path-selftest.js');

const required = prior.commands
  .filter(item => item.phase === 'REQUIRED')
  .map(item => item.command);
...
function assertionCount(output) {
  return Array.from(String(output || '').matchAll(
    /(?:SUMMARY\s+|RESULT\s+)?(\d+)\s+(?:focused\s+)?(?:assertions?|checks?|evidence groups?|controls?)\b/gi
...
function execute(command, phase) {
  ...
  const diagnostic = exitCode === 0
    ? null
    : String(result.stderr || result.stdout || result.error || '').trim().slice(-2000);
