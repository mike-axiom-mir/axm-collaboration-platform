...
[
  'specialist_zip_lane_excluded', 'global_tools_index_untouched', 'capability_gap_before_compared',
  'first_focused_replay_fixture_failed_honestly', 'fresh_replay_fixture_corrected',
  'same_controller_counterexample_preserved', 'joint_package_replacement_counterexample_preserved',
  'parallel_v33_test_flake_observed', 'v33_test_flake_corrected',
  'optional_schema_validator_unavailable', 'browser_verification_not_applicable',
  'evidence_browser_wording_match_corrected',
...
check(receipt.telemetryAggregation.retainedRawLogs === false && receipt.telemetryAggregation.commandOutcomes === 52, 'curation retains outcomes without raw logs');
check(receipt.telemetryAggregation.failedCommands === 0 && receipt.telemetryAggregation.focusedAssertions === 4629, 'curation retains exact verification totals');
...
check(index.openEvidence.includes('actual separate retention of checkpoint witness anchored receipt or expected anchor'), 'retention remains open');
...
check(sources.sources.length === 286 && sources.sources.every(item => {
...
check(results.summary.focusedAssertions === 4629, 'focused assertion count remains exact');
check(routes.routes.length === 5 && routes.routes.every(route => route.verdict === 'PASS'), 'all five evidence routes retain PASS verdicts');

console.log('\nReview-outcome history checkpoint anchor verification evidence: PASS (' + checks + ' checks)');
