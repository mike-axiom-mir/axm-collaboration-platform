...
const SURVEY_CONTRACT = Object.freeze({
  version: 'immutable-research-exam-to-bounded-document-and-session-segments-read-only-independent-declaration-pattern-survey-v3',
  ...
  allDocumentsCovered: true,
  sessionStorage: 'HASH_CHAINED_JSONL_PER_PROVIDER_ARCHITECTURE_SURVEY_BATCH_V1',
...
function discoverDocuments(workshopRoot) {
  ...
  function visit(directory) {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) visit(target);
        ...
        if (stat.size > MAX_DOCUMENT_BYTES) oversized.push({ relativePath: rel, bytes: stat.size, state: 'OVERSIZED_DOCUMENT_REFUSED_NOT_READ' });
          ...
          if (totalDocumentBytes + stat.size > MAX_TOTAL_DOCUMENT_BYTES) {
            throw new Error(`provider declaration architecture survey holds: JSON document bytes exceed the ${MAX_TOTAL_DOCUMENT_BYTES} total byte bound`);
          }
      ...
      if (documents.length + oversized.length > MAX_TOTAL_DOCUMENTS) throw new Error(`provider declaration architecture survey holds: more than ${MAX_TOTAL_DOCUMENTS} JSON documents across ${MAX_DOCUMENT_SEGMENTS} bounded segments`);
    }
  }
  visit(sharedRoot);
...
function planDocumentSegments(documents) {
  const segmentation = BoundedSegment.build(documents, {
    subject: 'provider-declaration-architecture-survey-documents',
  ...
  if (segmentation.plan.segmentCount > MAX_DOCUMENT_SEGMENTS) {
    throw new Error(`provider declaration architecture survey holds: ${segmentation.plan.segmentCount} document segments exceed ${MAX_DOCUMENT_SEGMENTS}`);
  }
  return segmentation;
}
...
function expectedSummary(results, inventory, sessionEvidence = null, includeSessionMetrics = true) {
  ...
  const summary = {
    ...
    documentSegmentCount: inventory.documentSegments.segmentCount,
    maximumDocumentsPerSegment: MAX_DOCUMENTS,
    allInventoryDocumentsCovered: inventory.documentSegments.allItemsCovered && inventory.documentSegments.deferredItems === 0,
    inventoryDocumentBytes: inventory.totalDocumentBytes,
...
function verifySessionEvidence(batch, runDir) {
  ...
  if (evidence.cellId !== SessionEvidenceSegments.CELL_ID || evidence.storage !== SURVEY_CONTRACT.sessionStorage ||
    ...
    throw new Error('provider declaration architecture survey session evidence declaration mismatch');
  }
  ...
  if (!segment || segment.segmentIndex !== 0 || segment.segmentId !== expectedSegmentId ||
    ...
    throw new Error('provider declaration architecture survey session evidence segment lineage mismatch');
  }
  let checked = null;
  if (runDir) {
    const file = path.resolve(runDir, segment.file || '');
    if (!inside(runDir, file) || !fs.existsSync(file)) throw new Error(`provider declaration architecture survey session segment missing: ${expectedSegmentId}`);
    try { checked = SessionEvidenceSegments.verify(file, segment.manifest); }
    catch (error) { throw new Error(`provider declaration architecture survey session hash mismatch: ${expectedSegmentId}: ${error.message}`); }
  }
  ...
  for (const result of batch.results) {
    ...
    if (result.sessionFile !== segment.file || !reference || !Number.isInteger(reference.index) ||
      ...
      throw new Error(`provider declaration architecture survey session reference mismatch: ${result.reasoningSessionId}`);
    }
    if (referenced.has(reference.index)) throw new Error('provider declaration architecture survey session record is referenced more than once');
    ...
    if (checked) {
      ...
      if (record.recordId !== reference.recordId || record.payloadDigest !== reference.payloadDigest || record.eventHash !== reference.eventHash) {
        throw new Error(`provider declaration architecture survey session record lineage mismatch: ${result.reasoningSessionId}`);
      }
      ...
      if (digest(bytes) !== result.sessionSha256 || record.payload.reasoningSessionId !== result.reasoningSessionId) {
        throw new Error(`provider declaration architecture survey session payload hash mismatch: ${result.reasoningSessionId}`);
      }
      verifySession(record.payload, result);
    }
  }
  if (referenced.size !== evidence.records) throw new Error('provider declaration architecture survey session evidence contains unreferenced records');
...
function verifyBatch(batch, runDir, researchDerived, discovered) {
  ...
  if (!same(batch.summary, expectedSummary(batch.results, batch.inventory, batch.sessionEvidence || null, includeSessionMetrics))) throw new Error('provider declaration architecture survey summary mismatch');
  return true;
}
...
function derive(options = {}) {
  ...
  const batch = {
    ...
    boundary: 'The organ may read bounded JSON documents under Workshop shared, record content-digested per-document pattern assessments under ignored private state, and preserve missing relations. Reasoning sessions share one sealed hash-chained JSONL segment per batch without rewriting historical files. It cannot merge documents into a provider, infer identity, select or evaluate architecture, build or write a provider, touch Workshop, claim readiness, train, execute, grant, install, or promote.'
  };
...
module.exports = {
  ...
  digest, sourceLineage, discoverDocuments, planDocumentSegments, hypothesisRequirements, assessDocuments, summarizeHypothesis, createSurvey,
  truthfulActionId, reasoningInput, verifySession, expectedSummary, sessionSegmentId, verifySessionEvidence, verifySurvey, verifyBatch, derive, respond
};
