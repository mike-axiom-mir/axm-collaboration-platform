...
const IMPLEMENTATION_CONTRACT = Object.freeze({
  version: 'consumer-bound-provider-declaration-gap-to-nonbuilt-hand-session-segment-v2',
  ...
  sessionStorage: 'HASH_CHAINED_JSONL_PER_PROVIDER_DECLARATION_HAND_BATCH_V1',
  maximumSessionsPerSegment: MAX_ASSESSMENTS,
...
function verifySessionEvidence(batch, runDir) {
  ...
  if (evidence.cellId !== SessionEvidenceSegments.CELL_ID || evidence.storage !== IMPLEMENTATION_CONTRACT.sessionStorage ||
    ...
    throw new Error('provider declaration hand session evidence declaration mismatch');
  }
  ...
  if (!segment || segment.segmentIndex !== 0 || segment.segmentId !== expectedSegmentId ||
    ...
    throw new Error('provider declaration hand session evidence segment lineage mismatch');
  }
  let checked = null;
  if (runDir) {
    const file = path.resolve(runDir, segment.file || '');
    if (!inside(runDir, file) || !fs.existsSync(file)) throw new Error(`provider declaration hand session segment missing: ${expectedSegmentId}`);
    try { checked = SessionEvidenceSegments.verify(file, segment.manifest); }
    catch (error) { throw new Error(`provider declaration hand session hash mismatch: ${expectedSegmentId}: ${error.message}`); }
  }
  ...
  for (const result of batch.results) {
    ...
    if (result.sessionFile !== segment.file || !reference || !Number.isInteger(reference.index) ||
      ...
      throw new Error(`provider declaration hand session reference mismatch: ${result.reasoningSessionId}`);
    }
    if (referenced.has(reference.index)) throw new Error('provider declaration hand session record is referenced more than once');
    ...
    if (checked) {
      ...
      if (record.recordId !== reference.recordId || record.payloadDigest !== reference.payloadDigest || record.eventHash !== reference.eventHash) {
        throw new Error(`provider declaration hand session record lineage mismatch: ${result.reasoningSessionId}`);
      }
      ...
      if (digest(bytes) !== result.sessionSha256 || record.payload.reasoningSessionId !== result.reasoningSessionId) {
        throw new Error(`provider declaration hand session payload hash mismatch: ${result.reasoningSessionId}`);
      }
      verifySession(record.payload, result);
    }
  }
  if (referenced.size !== evidence.records) throw new Error('provider declaration hand session evidence contains unreferenced records');
...
function verifyBatch(batch, runDir, affordanceBatch) {
  ...
  if (JSON.stringify(stable(batch.summary)) !== JSON.stringify(stable(expectedSummary(batch.results, batch.sessionEvidence || null, includeSessionMetrics)))) throw new Error('provider declaration hand summary mismatch');
  return true;
}
...
function derive(options = {}) {
  ...
  const batch = {
    ...
    boundary: 'The planner may originate a nonbuilt declaration-gap hand from exact consumer-bound demand when zero providers are declared. Reasoning sessions share one sealed hash-chained JSONL segment per batch without rewriting historical files. It never selects a provider, schema, implementation, or target path and never builds, writes, installs, probes, grants, trains, promotes, or changes Workshop.'
  };
...
module.exports = {
  ...
  digest, sourceLineage, createHandRequest, verifyHandRequest, truthfulActionId, reasoningInput, verifySession, expectedSummary, sessionSegmentId, verifySessionEvidence, verifyBatch, derive, respond
};
