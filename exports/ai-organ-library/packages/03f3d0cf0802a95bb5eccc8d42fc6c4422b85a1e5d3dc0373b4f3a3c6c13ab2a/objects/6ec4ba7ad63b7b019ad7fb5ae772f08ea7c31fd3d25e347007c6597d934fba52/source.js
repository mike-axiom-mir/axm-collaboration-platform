...
const IMPLEMENTATION_CONTRACT = Object.freeze({
  version: 'provider-declaration-gap-hand-to-independent-research-exam-session-segment-v2',
  ...
  sessionStorage: 'HASH_CHAINED_JSONL_PER_PROVIDER_RESEARCH_EXAM_BATCH_V1',
...
function verifySessionEvidence(batch, runDir) {
  ...
  if (evidence.cellId !== SessionEvidenceSegments.CELL_ID || evidence.storage !== IMPLEMENTATION_CONTRACT.sessionStorage ||
    ...
    throw new Error('provider declaration research exam session evidence declaration mismatch');
  }
  ...
  if (!segment || segment.segmentIndex !== 0 || segment.segmentId !== expectedSegmentId ||
    ...
    throw new Error('provider declaration research exam session evidence segment lineage mismatch');
  }
  let checked = null;
  if (runDir) {
    const file = path.resolve(runDir, segment.file || '');
    if (!inside(runDir, file) || !fs.existsSync(file)) throw new Error(`provider declaration research exam session segment missing: ${expectedSegmentId}`);
    try { checked = SessionEvidenceSegments.verify(file, segment.manifest); }
    catch (error) { throw new Error(`provider declaration research exam session hash mismatch: ${expectedSegmentId}: ${error.message}`); }
  }
  ...
  for (const result of batch.results) {
    ...
    if (result.sessionFile !== segment.file || !reference || !Number.isInteger(reference.index) ||
      ...
      throw new Error(`provider declaration research exam session reference mismatch: ${result.reasoningSessionId}`);
    }
    if (referenced.has(reference.index)) throw new Error('provider declaration research exam session record is referenced more than once');
    ...
    if (checked) {
      ...
      if (record.recordId !== reference.recordId || record.payloadDigest !== reference.payloadDigest || record.eventHash !== reference.eventHash) {
        throw new Error(`provider declaration research exam session record lineage mismatch: ${result.reasoningSessionId}`);
      }
      ...
      if (digest(bytes) !== result.sessionSha256 || record.payload.reasoningSessionId !== result.reasoningSessionId) {
        throw new Error(`provider declaration research exam session payload hash mismatch: ${result.reasoningSessionId}`);
      }
      verifySession(record.payload, result);
    }
  }
  if (referenced.size !== evidence.records) throw new Error('provider declaration research exam session evidence contains unreferenced records');
...
function verifyBatch(batch, runDir, providerDerived) {
  ...
  if (JSON.stringify(stable(batch.summary)) !== JSON.stringify(stable(expectedSummary(batch.results, batch.sessionEvidence || null, includeSessionMetrics)))) throw new Error('provider declaration research exam summary mismatch');
  return true;
}
...
function derive(options = {}) {
  ...
  const batch = {
    ...
    boundary: 'The planner may originate an immutable independent research-exam request plus one sealed session segment per bounded batch from a verified provider-declaration hand. It does not select an architecture, let a future candidate author its own expected outcomes, rewrite history, delete evidence, build fixtures or executors, construct a provider, write Workshop, claim readiness, grant, train, execute, install, or promote.'
  };
...
module.exports = {
  ...
  sessionSegmentId, verifySessionEvidence, verifyBatch, derive, respond
};
