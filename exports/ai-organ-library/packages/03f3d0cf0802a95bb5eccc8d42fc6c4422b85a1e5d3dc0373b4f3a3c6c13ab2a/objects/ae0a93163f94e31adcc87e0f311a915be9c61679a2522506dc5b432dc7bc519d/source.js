...
const AFFORDANCE_CONTRACT = Object.freeze({
  version: 'exact-provider-declaration-to-review-packet-session-segment-v3',
  ...
  sessionStorage: 'HASH_CHAINED_JSONL_PER_AFFORDANCE_BATCH_V1',
  maximumSessionsPerSegment: MAX_HANDS,
...
function verifySessionEvidence(batch, runDir) {
  ...
  if (evidence.cellId !== SessionEvidenceSegments.CELL_ID || evidence.storage !== AFFORDANCE_CONTRACT.sessionStorage ||
    ...
    throw new Error('readiness affordance session evidence declaration mismatch');
  }
  ...
  if (!segment || segment.segmentIndex !== 0 || segment.segmentId !== expectedSegmentId ||
    ...
    throw new Error('readiness affordance session evidence segment lineage mismatch');
  }
  let checked = null;
  if (runDir) {
    const file = path.resolve(runDir, segment.file || '');
    if (!inside(runDir, file) || !fs.existsSync(file)) throw new Error(`readiness affordance session segment missing: ${expectedSegmentId}`);
    try { checked = SessionEvidenceSegments.verify(file, segment.manifest); }
    catch (error) { throw new Error(`readiness affordance session hash mismatch: ${expectedSegmentId}: ${error.message}`); }
  }
  ...
  for (const result of batch.results) {
    ...
    if (result.sessionFile !== segment.file || !reference || !Number.isInteger(reference.index) ||
      ...
      throw new Error(`readiness affordance session reference mismatch: ${result.reasoningSessionId}`);
    }
    if (referenced.has(reference.index)) throw new Error('readiness affordance session record is referenced more than once');
    ...
    if (checked) {
      ...
      if (record.recordId !== reference.recordId || record.payloadDigest !== reference.payloadDigest || record.eventHash !== reference.eventHash) {
        throw new Error(`readiness affordance session record lineage mismatch: ${result.reasoningSessionId}`);
      }
      ...
      if (digest(bytes) !== result.sessionSha256 || record.payload.reasoningSessionId !== result.reasoningSessionId) {
        throw new Error(`readiness affordance session payload hash mismatch: ${result.reasoningSessionId}`);
      }
      verifySession(record.payload, result);
    }
  }
  if (referenced.size !== evidence.records) throw new Error('readiness affordance session evidence contains unreferenced records');
...
function verifyBatch(batch, runDir, handBatch, handoffBatch, workshopRoot) {
  ...
  if (JSON.stringify(stable(batch.summary)) !== JSON.stringify(stable(expectedSummary(batch.results, batch.sessionEvidence || null, includeSessionMetrics)))) throw new Error('readiness affordance summary mismatch');
  return true;
}
...
function derive(options = {}) {
  ...
  const sessionEvidence = {
    ...
    storage: AFFORDANCE_CONTRACT.sessionStorage,
...
module.exports = {
  ...
  digest, sourceLineage, safeRelative, resolvedRegularFile, moduleEvidence, walkServiceContracts, serviceInventory, consumerBindings, recommendation, assessHand, reasoningInput, verifySession, expectedSummary, sessionSegmentId, verifySessionEvidence, verifyBatch, derive, respond
};
