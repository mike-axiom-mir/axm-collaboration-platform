...
function sourceFileDeltaCount(currentSubject, priorSubject) {
  const current = new Map((((currentSubject || {}).files) || []).map(item => [item.path, item.sha256]));
  const prior = new Map((((priorSubject || {}).files) || []).map(item => [item.path, item.sha256]));
  return Array.from(new Set([...current.keys(), ...prior.keys()])).filter(file => current.get(file) !== prior.get(file)).length;
}

function evidenceSectionDeltaCount(currentEvidence, priorEvidence) {
  const current = currentEvidence || {};
  const prior = priorEvidence || {};
  return Array.from(new Set([...Object.keys(current), ...Object.keys(prior)])).filter(key => !same(current[key], prior[key])).length;
}

function selectTransitionWitness(dimension, candidates, currentSubject, currentEvidence) {
  const scored = candidates.map(({ prior, old }) => ({
    prior,
    old,
    sourceFilesChanged: sourceFileDeltaCount(currentSubject, prior.subject),
    evidenceSectionsChanged: evidenceSectionDeltaCount(currentEvidence, prior.evidence)
  })).sort((left, right) =>
    left.sourceFilesChanged - right.sourceFilesChanged ||
    left.evidenceSectionsChanged - right.evidenceSectionsChanged ||
    left.prior.snapshotDigest.localeCompare(right.prior.snapshotDigest));
  const selected = scored[0];
  return {
    priorSnapshotId: selected.prior.snapshotId,
    dimensionId: dimension.id,
    previousState: selected.old.state,
    currentState: dimension.state,
    witnessSelection: {
      rule: 'MINIMUM_SOURCE_FILE_DELTA_THEN_EVIDENCE_SECTION_DELTA_THEN_SNAPSHOT_DIGEST',
      eligiblePriorSnapshots: scored.length,
      sourceFilesChanged: selected.sourceFilesChanged,
      evidenceSectionsChanged: selected.evidenceSectionsChanged,
      inputOrderAuthority: false,
      recencyClaimed: false
    }
  };
}

function transitionRows(currentDimensions, relevant, currentSubject, currentEvidence, eligible) {
  const rows = [];
  for (const dimension of currentDimensions) {
    const candidates = [];
    for (const prior of relevant) {
      const old = (prior.dimensions || []).find(item => item.id === dimension.id);
      if (old && eligible(old.state, dimension.state)) candidates.push({ prior, old });
    }
    if (candidates.length) rows.push(selectTransitionWitness(dimension, candidates, currentSubject, currentEvidence));
  }
  return rows.sort((left, right) => left.dimensionId.localeCompare(right.dimensionId));
}

function comparePrior(currentDimensions, priorSnapshots, currentSubject, currentEvidence) {
  const currentSubjectDigest = currentSubject && currentSubject.digest;
  const currentEvidenceDigest = digest(currentEvidence);
  ...
  const regressions = transitionRows(currentDimensions, relevant, currentSubject, currentEvidence,
    (old, current) => old === 'OBSERVED_PASS' && current === 'REGRESSION_REQUIRES_REVIEW');
  const improvements = transitionRows(currentDimensions, relevant, currentSubject, currentEvidence,
    (old, current) => old.startsWith('HOLD_') && current === 'OBSERVED_PASS');
  const recoveries = transitionRows(currentDimensions, relevant, currentSubject, currentEvidence,
    (old, current) => old === 'REGRESSION_REQUIRES_REVIEW' && current === 'OBSERVED_PASS');
  const evidenceContinuityLosses = transitionRows(currentDimensions, relevant, currentSubject, currentEvidence,
    (old, current) => old === 'OBSERVED_PASS' && current.startsWith('HOLD_'));
  return {
...
function snapshotState(sourceMode, directRegressions, comparison) {
  if (sourceMode === 'SYNTHETIC_OBSERVATORY_FIXTURE') return 'SYNTHETIC_OBSERVATORY_FIXTURE_NOT_OPERATIONAL_EVIDENCE';
  if ((directRegressions || []).length) return 'HOLD_FOUNDATION_BEHAVIOR_REGRESSION_REQUIRES_REVIEW';
  if (((comparison || {}).evidenceContinuityLosses || []).length) return 'HOLD_FOUNDATION_EVIDENCE_CONTINUITY_REQUIRES_REVIEW';
  return comparison && comparison.priorVerifiedSubjects
...
function buildSnapshot(evidence, priorSnapshots = []) {
  ...
  const summarizedEvidence = evidenceSummary(evidence);
  const currentEvidenceDigest = digest(summarizedEvidence);
  const comparison = comparePrior(measured, priorSnapshots, evidence.subject, summarizedEvidence);
  ...
  const snapshot = {
    ...
    comparison,
    openGates,
    state: snapshotState(evidence.sourceMode, directRegressions, comparison),
    authority: {
...
module.exports = {
  ...
  evidenceSummary, sourceFileDeltaCount, evidenceSectionDeltaCount, comparePrior, snapshotState,
  buildSnapshot, verifySnapshot, loadPriorSnapshots, run
};
