...
const game=fs.readFileSync(path.join(root,'tools/game-hub/game-night.js'),'utf8');
...
const checks=[
  ...
  [core.includes("definition.evidence")&&core.includes("receipt requires evidence"),'aggregate receipt evidence gate'],
  [core.includes("sfxCreated")&&core.includes("assetsCreated")&&core.includes("gamesDeveloped"),'extended creative and build metric families'],
  [core.includes("kill-recorded")&&core.includes("death-recorded"),'verified combat result metrics'],
  [core.includes("LEGACY_ACHIEVEMENT_ALIASES")&&core.includes("games-played-10"),'overlapping starter achievements migrate without duplicates'],
  ...
  [server.includes('PROFILE_STATE_DIR')&&server.includes('shared-profile')&&server.includes('PROFILE_STATE_FILE')&&server.includes('profile.json'),'local persistence'],
  [server.includes('/api/profile/opt-in'),'explicit opt-in API'],
  [server.includes('/api/profile/health')&&server.includes('/api/profile/receipt'),'global receipt health and module adapter'],
  [server.includes('delete-local-profile'),'explicit delete API'],
  [client.includes("'/api/profile/event'")&&client.includes("'/api/profile/receipt'"),'shared browser activity adapters'],
  [!client.includes("var human = members.find"),'unmatched module work is never silently credited to the human'],
  ...
  [fs.existsSync(path.join(root,'assets/achievements/manifest.json'))&&fs.existsSync(path.join(root,'assets/achievements/achievement-code-characters-100000-primal.png')),'professional artwork pack and manifest'],
  [game.includes("type:'game-played'")&&game.includes("dedupeKey:'game-result:'"),'explicit finished game receipt'],
