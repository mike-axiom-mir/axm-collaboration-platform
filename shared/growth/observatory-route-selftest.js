...
assert.ok(server.includes('observatoryRunner: current.stats'));
...
assert.ok(hub.includes('growthOpportunitySummary'));
assert.ok(hub.includes('data-growth-filter="CONTEXT"'));
assert.ok(client.includes('/api/workshop-observatory'));
assert.ok(client.includes('automaticallyProven'));
assert.ok(client.includes('STALE SNAPSHOT'));
