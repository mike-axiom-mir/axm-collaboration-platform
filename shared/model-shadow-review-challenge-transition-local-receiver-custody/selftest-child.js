...
const service = Custody.createReceiver(input.options);
let output;
if (input.action === 'receive') output = service.receive(input.input);
else if (input.action === 'reload') output = service.reload(input.input);
else throw new Error('child action must be receive or reload');
