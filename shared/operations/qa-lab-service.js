...
function create(options) {
  ...
  function normalizePhoneObservation(input) {
    ...
    if (!input || typeof input !== 'object' || Array.isArray(input)) throw new Error('phone observation must be an object');
    const gameId = String(input.gameId || '').trim().toLowerCase();
    const slot = String(input.slot || '').trim();
    if (!/^\d{3}-[a-z0-9][a-z0-9-]{0,95}$/.test(gameId)) throw new Error('phone observation needs a bounded game id');
    if (slot && !/^\d{3}$/.test(slot)) throw new Error('phone observation slot must be three digits');
    if (slot && gameId.slice(0, 3) !== slot) throw new Error('phone observation slot must match the game id');
    const observations = Object.fromEntries(PHONE_OBSERVATION_KEYS.map(key => [key, input[key] === true]));
    const complete = PHONE_OBSERVATION_KEYS.every(key => observations[key]);
    return {
      schema: PHONE_OBSERVATION_SCHEMA,
      gameId,
      slot: slot || gameId.slice(0, 3),
      observations,
  ...
  function recordDeviceEvidence(input) {
    ...
    const receipt = { schema: 'axm.device-qa-evidence/v1', id: U.uid('device-qa'), capturedAt: U.now(), actor: String(body.actor || 'local-user').slice(0, 120), userAgent: String(body.userAgent || '').slice(0, 400), viewport: { width: Number(viewport.width), height: Number(viewport.height), devicePixelRatio: Number(viewport.devicePixelRatio) || 1 }, accessibility: { reducedMotion: !!body.reducedMotion, highContrast: !!body.highContrast }, gamepads, network: { samples, medianMs: sorted.length ? sorted[Math.floor(sorted.length / 2)] : null, p95Ms: p95, disconnectObserved: body.disconnectObserved === true, recoveredAfterDisconnect: body.recoveredAfterDisconnect === true }, longSession: { durationMs: Math.max(0, Math.min(24 * 60 * 60 * 1000, Number(body.sessionDurationMs) || 0)), errors: (Array.isArray(body.errors) ? body.errors : []).slice(0, 50).map(x => String(x).slice(0, 300)) }, phoneObservation, truth: { hardwareEnumeratedByBrowser: true, rawInputStored: false, automaticPermissionChange: false, physicalHardwareProven: false, manifestMutated: false, externalReviewRequired: phoneObservation !== null } };
    receipt.digest = U.sha256(JSON.stringify(receipt)); const state = read(); state.deviceEvidence.unshift(receipt); state.deviceEvidence = state.deviceEvidence.slice(0, 100); write(state); audit({ type: 'device-evidence', id: receipt.id, gamepads: gamepads.length, gameId: phoneObservation ? phoneObservation.gameId : null, phoneObservationComplete: phoneObservation ? phoneObservation.complete : false, digest: receipt.digest }); return receipt;
  }
  function status() { const state = read(); const phoneCandidates = state.deviceEvidence.filter(item => item && item.phoneObservation); return { schema: SCHEMA, profiles: Object.keys(PROFILE_STEPS), journeys: state.journeys, deviceEvidence: state.deviceEvidence, phoneObservationCandidates: phoneCandidates.length, latestJourney: state.journeys[0] || null, latestDeviceEvidence: state.deviceEvidence[0] || null, latestPhoneObservation: phoneCandidates[0] || null, arbitraryUrlTesting: false, hardwarePermissionAuthority: false }; }
  return { status, run, recordDeviceEvidence, normalizePhoneObservation, normalizeSteps, inspectHtml, stateFile, auditFile };
}

module.exports = { SCHEMA, PHONE_OBSERVATION_SCHEMA, PHONE_OBSERVATION_KEYS, PROFILE_STEPS, create };
