...
function create(options) {
  ...
  function upsert(input) {
    ...
    const scopes = Array.from(new Set((Array.isArray(input.scopes) ? input.scopes : []).map(x => String(x).trim()).filter(Boolean))).slice(0, 30); if (!scopes.length) throw new Error('at least one consumer scope is required');
    const records = metadata().filter(item => item.id !== id), existing = metadata().find(item => item.id === id); values[id] = value;
    const record = { id, label: String(input.label || id).slice(0, 120), scopes, expiresAt: input.expiresAt || null, fingerprint: U.sha256(Buffer.from(value)).slice(0, 12), createdAt: existing ? existing.createdAt : U.now(), rotatedAt: existing ? U.now() : null, updatedAt: U.now(), revoked: false };
  ...
  function readSecret(id, consumer) {
    ...
    if (!record || record.revoked || !record.scopes.includes(consumer) || (record.expiresAt && Date.parse(record.expiresAt) <= Date.now())) { audit({ type: 'access-refused', id, consumer }); throw new Error('secret access refused'); }
  ...
  function status() { const vault = readVault(); return { initialized: !!vault, unlocked: !!key && Date.now() < unlockedUntil, unlockedUntil: key ? new Date(unlockedUntil).toISOString() : null, cipher: vault ? vault.cipher : null, records: metadata().map(U.clone), valuesExposedToBrowser: false }; }
  return { initialize, unlock, lock, upsert, revoke, readSecret, status, vaultFile, auditFile };
}

module.exports = { SCHEMA, derive, encrypt, decrypt, create };
