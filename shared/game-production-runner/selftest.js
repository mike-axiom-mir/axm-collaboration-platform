...
async function main() {
  ...
  try {
    ...
    const cacheLeaseRollupReceiptSchema = JSON.parse(fs.readFileSync(path.join(__dirname, 'schemas', 'production-artifact-cache-lease-archive-rollup-receipt.schema.json'), 'utf8'));
    ...
    const rollupRoot = path.join(temporary, 'lease-rollup-cache'), rollupJobs = path.join(temporary, 'lease-rollup-jobs');
    fs.mkdirSync(rollupJobs);
    Core.artifactCache.open({ cacheRoot: rollupRoot, sourceRoot, jobRoot: rollupJobs });
    const rollupOptions = { cacheRoot: rollupRoot, sourceRoot, jobRoot: rollupJobs, runId: 'lease-rollup-run', planDigest: '4'.repeat(64), startedAt: 'lease-rollup-start', durationMs: 1000 };
    let rollupTime = 50000000, rollupLeaseId = null;
    function archiveRollupCycle(key) {
      const session = Core.cacheLeases.acquire(Object.assign({}, rollupOptions, { nowMs: rollupTime }));
      session.protect(key, rollupTime + 1);
      session.release(rollupTime + 2);
      rollupLeaseId = session.leaseId;
      const set = Core.cacheLeases.discover({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 3 });
      const proposal = Core.cacheLeaseCuration.plan(set, { minimum_released_age_ms: 0, minimum_expired_age_ms: 0, max_candidates: 10 });
      const applied = Core.cacheLeaseCuration.apply({ cacheRoot: rollupRoot, sourceRoot, proposal, approvedDigest: proposal.digest, explicit: true, nowMs: rollupTime + 3 });
      equal(applied.status, 'APPLIED', 'rollup fixture archives one exact released live segment');
      rollupTime += 10;
      return session.event;
    }
    for (const key of ['4', '5', '6', '7', '8'].map((value) => value.repeat(64))) archiveRollupCycle(key);
    const preRollupLeaseSet = Core.cacheLeases.discover({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime });
    const preRollupAudit = Core.cacheLeases.auditArchives({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime });
    ok(preRollupAudit.status === 'COMPLETE' && preRollupAudit.usage.segments === 5 && preRollupAudit.usage.archive_files === 10, 'five independently archived generations have a complete ten-file cold audit before rollup');
    const rollupPolicy = { minimum_released_age_ms: 0, minimum_expired_age_ms: 0, minimum_reclaim_bytes: 1, max_candidates: 10 };
    const firstRollupPlanResult = Core.cacheLeaseRollup.plan({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy), firstRollupPlan = firstRollupPlanResult.proposal, firstRollupCandidate = firstRollupPlan.candidates[0];
    ok(firstRollupPlan.status === 'READY' && firstRollupPlan.candidates.length === 1 && firstRollupCandidate.projected_reduction_bytes > 0 && !firstRollupPlan.authority.source_archive_removal, 'rollup dry-run selects only a measured smaller target and carries no source-removal authority');
    const reshapedRollupPlan = Core.canonical.seal(Object.assign({}, firstRollupPlan, { undeclared_rollup_authority: true }));
    throws(() => Core.cacheLeaseRollup.validateProposal(reshapedRollupPlan), /CACHE_LEASE_ARCHIVE_ROLLUP_PROPOSAL_INVALID/, 'rollup rejects a newly sealed proposal with undeclared authority fields');
    const wrongRollupApproval = Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: firstRollupPlan, approvedDigest: '0'.repeat(64), explicit: true, nowMs: rollupTime + 1 });
    ok(wrongRollupApproval.status === 'APPROVAL_MISMATCH' && Core.cacheLeases.auditArchives({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 1 }).usage.archive_files === 10, 'wrong rollup approval removes no archive source file');
    const busyRollup = Core.cacheLeases.withLeaseLock(rollupOptions, rollupLeaseId, () => Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: firstRollupPlan, approvedDigest: firstRollupPlan.digest, explicit: true, nowMs: rollupTime + 1 }));
    equal(busyRollup.status, 'COORDINATION_BUSY', 'lease lifecycle coordination refuses rollup while the owner is busy');
    const copiedRollupRoot = path.join(temporary, 'copied-rollup-cache');
    fs.cpSync(rollupRoot, copiedRollupRoot, { recursive: true });
    const copiedRollup = Core.cacheLeaseRollup.apply({ cacheRoot: copiedRollupRoot, sourceRoot, proposal: firstRollupPlan, approvedDigest: firstRollupPlan.digest, explicit: true, nowMs: rollupTime + 1 });
    equal(copiedRollup.status, 'STALE', 'rollup approval is bound to the exact observed cache root');
    const partialTargetPlanResult = Core.cacheLeaseRollup.plan({ cacheRoot: copiedRollupRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy), partialTargetPlan = partialTargetPlanResult.proposal;
    const rollupBuilderRoot = path.join(temporary, 'rollup-target-builder-cache');
    fs.cpSync(rollupRoot, rollupBuilderRoot, { recursive: true });
    const builderPlanResult = Core.cacheLeaseRollup.plan({ cacheRoot: rollupBuilderRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy), builderPlan = builderPlanResult.proposal;
    equal(builderPlan.candidates[0].target_rollup_receipt_digest, partialTargetPlan.candidates[0].target_rollup_receipt_digest, 'deterministic rollup target is independent of copied-root proposal authority');
    equal(Core.cacheLeaseRollup.apply({ cacheRoot: rollupBuilderRoot, sourceRoot, proposal: builderPlan, approvedDigest: builderPlan.digest, explicit: true, nowMs: rollupTime + 1 }).status, 'APPLIED', 'disposable builder materializes the exact deterministic target');
    const partialTargetDigest = partialTargetPlan.candidates[0].target_rollup_receipt_digest, builderArchiveRoot = path.join(rollupBuilderRoot, 'lease-archives', rollupLeaseId), partialArchiveRoot = path.join(copiedRollupRoot, 'lease-archives', rollupLeaseId);
    fs.copyFileSync(path.join(builderArchiveRoot, partialTargetDigest + '.jsonl.gz'), path.join(partialArchiveRoot, partialTargetDigest + '.jsonl.gz'), fs.constants.COPYFILE_EXCL);
    const partialTargetRecovery = Core.cacheLeaseRollup.apply({ cacheRoot: copiedRollupRoot, sourceRoot, proposal: partialTargetPlan, approvedDigest: partialTargetPlan.digest, explicit: true, nowMs: rollupTime + 1 });
    ok(partialTargetRecovery.status === 'APPLIED' && Core.cacheLeases.auditArchives({ cacheRoot: copiedRollupRoot, sourceRoot, nowMs: rollupTime + 1 }).status === 'COMPLETE', 'exact replay reuses a partial target blob written before anchor publication and completes the remaining files');
    const rollupArchiveRoot = path.join(rollupRoot, 'lease-archives', rollupLeaseId), savedRollupSources = new Map(firstRollupCandidate.source_files.map((item) => [item.name, fs.readFileSync(path.join(rollupArchiveRoot, item.name))]));
    const firstRollup = Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: firstRollupPlan, approvedDigest: firstRollupPlan.digest, explicit: true, nowMs: rollupTime + 1 });
    const postRollupLeaseSet = Core.cacheLeases.discover({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 1 }), postRollupAudit = Core.cacheLeases.auditArchives({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 1 });
    ok(firstRollup.status === 'APPLIED' && firstRollup.source_files_removed === 10 && postRollupAudit.status === 'COMPLETE' && postRollupAudit.usage.segments === 1 && postRollupAudit.usage.archive_files === 3 && postRollupAudit.usage.storage_bytes < preRollupAudit.usage.storage_bytes, 'exact rollup replaces ten source files with three smaller files and preserves a complete one-segment audit');
    equal(postRollupLeaseSet.leases.map((item) => [item.tail_event_digest, item.generation, item.state, item.key_set_digest]), preRollupLeaseSet.leases.map((item) => [item.tail_event_digest, item.generation, item.state, item.key_set_digest]), 'rollup preserves hot discovery tail, generation, state, and key-set semantics');
    for (const [name, bytes] of savedRollupSources) fs.writeFileSync(path.join(rollupArchiveRoot, name), bytes, { flag: 'wx' });
    equal(Core.cacheLeases.auditArchives({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 1 }).status, 'REVIEW_REQUIRED', 'full audit exposes the interruption window after target publication but before source cleanup');
    const cleanupRaceSession = Core.cacheLeases.acquire(Object.assign({}, rollupOptions, { nowMs: rollupTime + 2 }));
    const recoveredRollup = Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: firstRollupPlan, approvedDigest: firstRollupPlan.digest, explicit: true, nowMs: rollupTime + 3 });
    ok(recoveredRollup.status === 'APPLIED' && recoveredRollup.outcomes[0].status === 'RECOVERED' && recoveredRollup.source_files_removed === 10, 'reapplying the exact proposal removes only covered sources even after a new live segment starts');
    cleanupRaceSession.protect('e'.repeat(64), rollupTime + 4);
    cleanupRaceSession.release(rollupTime + 5);
    const freshRollupCycle = cycleLeaseInChild({ module: require.resolve('./artifact-cache-lease'), options: Object.assign({}, rollupOptions, { nowMs: rollupTime + 6 }), key: '9'.repeat(64), protectAt: rollupTime + 7, releaseAt: rollupTime + 8 });
    ok(freshRollupCycle.acquired.generation === 19 && freshRollupCycle.released.generation === 21, 'a fresh process continues the consolidated rollup anchor without replaying separate archive files');
    rollupTime += 10;
    equal(Core.cacheLeaseRollup.plan({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy).proposal.status, 'NO_CHANGES', 'an unarchived live segment prevents archive rollup even after release');
    let rollupLiveSet = Core.cacheLeases.discover({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime });
    let rollupLivePlan = Core.cacheLeaseCuration.plan(rollupLiveSet, { minimum_released_age_ms: 0, minimum_expired_age_ms: 0, max_candidates: 10 });
    equal(Core.cacheLeaseCuration.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: rollupLivePlan, approvedDigest: rollupLivePlan.digest, explicit: true, nowMs: rollupTime }).status, 'APPLIED', 'fresh-process post-rollup history can return to the governed cold chain');
    rollupTime += 10;
    for (const key of ['a', 'b', 'c'].map((value) => value.repeat(64))) archiveRollupCycle(key);
    const staleRollupPlanResult = Core.cacheLeaseRollup.plan({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy), staleRollupPlan = staleRollupPlanResult.proposal;
    equal(staleRollupPlan.status, 'READY', 'repeated rollup remains measurable after new independently archived generations');
    const rollupRaceSession = Core.cacheLeases.acquire(Object.assign({}, rollupOptions, { nowMs: rollupTime + 1 }));
    rollupRaceSession.protect('d'.repeat(64), rollupTime + 2);
    const staleRollup = Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: staleRollupPlan, approvedDigest: staleRollupPlan.digest, explicit: true, nowMs: rollupTime + 2 });
    ok(staleRollup.status === 'STALE' && !staleRollup.authority.source_archive_files_removed, 'a new active lease session makes the exact rollup proposal stale before archive removal');
    rollupRaceSession.release(rollupTime + 3);
    rollupLiveSet = Core.cacheLeases.discover({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 4 });
    rollupLivePlan = Core.cacheLeaseCuration.plan(rollupLiveSet, { minimum_released_age_ms: 0, minimum_expired_age_ms: 0, max_candidates: 10 });
    equal(Core.cacheLeaseCuration.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: rollupLivePlan, approvedDigest: rollupLivePlan.digest, explicit: true, nowMs: rollupTime + 4 }).status, 'APPLIED', 'stale active history is archived only through a new exact curation proposal');
    rollupTime += 10;
    const repeatedRollupPlanResult = Core.cacheLeaseRollup.plan({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime }, rollupPolicy), repeatedRollupPlan = repeatedRollupPlanResult.proposal;
    const repeatedRollup = Core.cacheLeaseRollup.apply({ cacheRoot: rollupRoot, sourceRoot, proposal: repeatedRollupPlan, approvedDigest: repeatedRollupPlan.digest, explicit: true, nowMs: rollupTime + 1 });
    const repeatedRollupAudit = Core.cacheLeases.auditArchives({ cacheRoot: rollupRoot, sourceRoot, nowMs: rollupTime + 1 });
    ok(repeatedRollup.status === 'APPLIED' && repeatedRollupAudit.status === 'COMPLETE' && repeatedRollupAudit.usage.rollups === 1 && repeatedRollupAudit.usage.segments === 1 && repeatedRollupAudit.usage.history_events === 33, 'repeated rollup preserves thirty-three exact events behind one current rollup head');
    const rollupReceiptName = fs.readdirSync(rollupArchiveRoot).find((name) => name.endsWith('.rollup.json')), rollupReceipt = JSON.parse(fs.readFileSync(path.join(rollupArchiveRoot, rollupReceiptName), 'utf8'));
    ...
    equal(Core.cacheLeases.discover({ cacheRoot: missingLineageRoot, sourceRoot, nowMs: rollupTime + 1 }).status, 'REVIEW_REQUIRED', 'routine discovery checks that the exact lineage blob named by a rollup head still exists');
    const tamperedLineageRoot = path.join(temporary, 'tampered-rollup-lineage-cache');
    fs.cpSync(rollupRoot, tamperedLineageRoot, { recursive: true });
    const tamperedLineageFile = path.join(tamperedLineageRoot, 'lease-archives', rollupLeaseId, lineageName), tamperedLineageBytes = fs.readFileSync(tamperedLineageFile);
    tamperedLineageBytes[Math.floor(tamperedLineageBytes.length / 2)] ^= 1;
    fs.writeFileSync(tamperedLineageFile, tamperedLineageBytes);
    const tamperedLineageAudit = Core.cacheLeases.auditArchives({ cacheRoot: tamperedLineageRoot, sourceRoot, nowMs: rollupTime + 1 });
    ok(tamperedLineageAudit.status === 'REVIEW_REQUIRED' && !/[A-Za-z]:\\/.test(JSON.stringify(tamperedLineageAudit)) && !JSON.stringify(tamperedLineageAudit).includes('lease-rollup-run'), 'deep audit detects same-size lineage corruption without disclosing paths or raw run identity');
    ...
    const concurrentResults = await Promise.all(Array.from({ length: 8 }, () => publishCacheInChild(concurrentPayload)));
    equal(concurrentResults.map((item) => item.state).sort(), ['EXISTS', 'EXISTS', 'EXISTS', 'EXISTS', 'EXISTS', 'EXISTS', 'EXISTS', 'STORED'], 'eight fresh concurrent cache publishers converge on one immutable cache entry');
