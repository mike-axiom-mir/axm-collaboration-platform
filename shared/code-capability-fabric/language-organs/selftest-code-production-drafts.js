...
const draft1v3 = production.reviseDraft({
  ...
  draft: draft1v2,
  actorClass: 'HUMAN',
  editSummary: 'Keep the candidate but revise naming and state boundaries.',
...
const comparison = production.compareDrafts({
  batch,
  drafts: [draft1v1, draft1v2, draft1v3, ...set.drafts.slice(1)]
});
assert.strictEqual(comparison.result, 'DRAFT_COMPARISON_READY');
assert.strictEqual(comparison.rows.length, 4);
assert.strictEqual(comparison.rows.find(r => r.draftId === 'draft-01').revision, 3);
...
const svg = production.renderComparisonSvg({
  batch,
  drafts: [draft1v3, ...set.drafts.slice(1)],
...
try {
  ...
  assert.strictEqual(productionStore.selectActiveDraftRevision({ root, draft: draft1v3, actorClass: 'HUMAN' }).operation, 'SELECT_ACTIVE_DRAFT_REVISION');
  assert.strictEqual(productionStore.putSelection({ root, selection }).operation, 'PUT_SELECTION_POINTER');
  ...
  assert.strictEqual(storedDraft1.revisions.length, 3);
  assert.strictEqual(storedDraft1.active.draftRevisionSha256, draft1v3.draftRevisionSha256);
  assert.strictEqual(read.selection.draftRevisionSha256, draft1v3.draftRevisionSha256);
  assert.strictEqual(read.truth.rawSourceStored, false);

  const fakeRaw = {
    ...draft1v3,
    draftRevisionSha256: 'sha256:fake-raw-revision',
    keyProgram: { ...program, sourceCode: 'console.log("raw")' }
  };
