...
test('catalog contains exactly the seven bounded reviewed lanes', () => assert.deepStrictEqual(Registry.CATALOG.profiles.map((row) => row.id), ['code-family.bounded-css-token-stylesheet', 'code-family.bounded-javascript-record-transform', 'code-family.bounded-python-record-transform', 'code-family.closed-object-contract-adapter', 'code-family.json-schema-validator', 'code-family.static-html-page', 'code-family.svg-status-badge']));
...
test('same specialist and language may expose multiple abilities only through distinct exact bindings', () => { const draft = clone(Registry.CATALOG); delete draft.catalogDigest; const sealed = Registry.sealCatalog(draft); assert.strictEqual(sealed.profiles.filter((row) => row.specialistOrganRef.id === 'organ.code.application-logic' && row.languageIds.includes('javascript')).length, 2); });
