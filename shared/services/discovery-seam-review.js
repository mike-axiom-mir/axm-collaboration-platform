...
const checks=[
  ['Ten services exist once',()=>core.definitions.length===10&&new Set(core.definitions.map(x=>x.id)).size===10],
  ...
  ['Mirror installation and runtime are probed',()=>/\/api\/mirror-core\/status/.test(ui)&&/probeMirror/.test(ui)],
  ['Mirror runtime start requires an explicit action',()=>/x-axm-mirror-action/.test(ui)&&/explicit-start/.test(ui)],
