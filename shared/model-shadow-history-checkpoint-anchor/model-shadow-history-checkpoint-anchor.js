...
const WITNESS_POLICY_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-witness-policy/v1';
const WITNESS_ATTESTATION_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-witness-attestation/v1';
const WITNESS_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-witness/v1';
const ANCHOR_POLICY_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchor-policy/v1';
const ANCHOR_AUTHORIZATION_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchor-authorization/v1';
const ANCHOR_COMMITMENT_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchor-commitment/v1';
const ANCHORED_CHECKPOINT_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchored/v1';
const ANCHORED_AUDIT_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchored-audit/v1';
const SET_SCHEMA = 'axm.model-shadow-two-phase-history-checkpoint-anchor-set/v1';
const VERSION = '2.2.0';
const STATUS = 'TEST';
const WITNESS_AUDIENCE = 'MODEL_SHADOW_TWO_PHASE_HISTORY_CHECKPOINT_WITNESS';
const WITNESS_SCOPE = 'WITNESS_EXACT_MODEL_SHADOW_TWO_PHASE_HISTORY_CHECKPOINT';
const ANCHOR_AUDIENCE = 'MODEL_SHADOW_TWO_PHASE_HISTORY_CHECKPOINT_ANCHOR';
const ANCHOR_SCOPE = 'AUTHORIZE_EXACT_WITNESSED_MODEL_SHADOW_TWO_PHASE_HISTORY_CHECKPOINT';
...
function normalizeWitnessPolicy(input, checkpoint, verifiedAt) {
  ...
  exactKeys(policy, [
    'schema', 'policyId', 'issuedAt', 'expiresAt', 'audience', 'scope', 'authorityOrigin',
    'checkpointRef', 'requiredSignatures', 'maxAttestationAgeSeconds', 'keys', 'policyDigest'
  ...
  if (Date.parse(issuedAt) < Date.parse(checkpoint.checkpointedAt) || Date.parse(issuedAt) > Date.parse(verifiedAt)) {
    throw new Error('checkpoint witness policy time does not bracket checkpoint and verification');
  }
  if (Date.parse(expiresAt) <= Date.parse(verifiedAt) || Date.parse(expiresAt) <= Date.parse(issuedAt)) {
    throw new Error('checkpoint witness policy is expired or has an invalid validity window');
  }
  ...
  const keys = normalizeSeats(policy.keys, {
    label: 'checkpoint witness policy keys', keyIdField: 'keyId', principalField: 'actorDigest', kindField: 'actorKind', scope: WITNESS_SCOPE
  });
  ...
  return { policyId, issuedAt, expiresAt, ref, requiredSignatures, maxAttestationAgeSeconds, keys, policyDigest: expected };
}
...
function verifyWitnessAttestation(input, policy, checkpoint, verifiedAt, index) {
  ...
  exactKeys(item, [
    'schema', 'attestationId', 'keyId', 'actorDigest', 'actorKind', 'verdict', 'scope',
    'policyDigest', 'checkpointDigest', 'issuedAt', 'expiresAt', 'signatureAlgorithm', 'signature'
  ], label);
  ...
  const issuedAt = timestamp(item.issuedAt, label + '.issuedAt');
  const expiresAt = timestamp(item.expiresAt, label + '.expiresAt');
  const issuedMs = Date.parse(issuedAt); const expiresMs = Date.parse(expiresAt); const verifiedMs = Date.parse(verifiedAt);
  if (issuedMs < Date.parse(policy.issuedAt) || issuedMs < Date.parse(checkpoint.checkpointedAt) || issuedMs > verifiedMs) {
    throw new Error(label + ' issuance time is outside the exact policy/checkpoint window');
  }
  if (expiresMs <= issuedMs || expiresMs <= verifiedMs || expiresMs > Date.parse(policy.expiresAt) ||
      verifiedMs - issuedMs > policy.maxAttestationAgeSeconds * 1000 || expiresMs - issuedMs > policy.maxAttestationAgeSeconds * 1000) {
    throw new Error(label + ' is expired or exceeds the witness age window');
  }
  ...
  return {
    attestationId, attestationDigest: sha256(item), keyId,
    fingerprint: seat.fingerprint, principalDigest: seat.principalDigest, kind: seat.kind, issuedAt, expiresAt
  };
}
...
function buildWitnessDetailed(input) {
  ...
  if (!Array.isArray(input.signedAttestations) || input.signedAttestations.length < policy.requiredSignatures || input.signedAttestations.length > MAX_SEATS) {
    throw new Error('checkpoint witness attestations must cover the threshold and contain at most ten entries');
  }
  ...
  seats.forEach(seat => {
    ...
    if (keys.has(seat.keyId) || fingerprints.has(seat.fingerprint) || principals.has(seat.principalDigest)) {
      throw new Error('checkpoint witness signatures must occupy unique key and declared-principal seats');
    }
    ids.add(seat.attestationId); keys.add(seat.keyId); fingerprints.add(seat.fingerprint); principals.add(seat.principalDigest);
  });
  const fingerprintValues = seats.map(seat => seat.fingerprint);
  const principalValues = seats.map(seat => seat.principalDigest);
  const receipt = {
    ...
    signatureEvidence: {
      ...
      verifiedSignatures: seats.length,
      keyFingerprintSetDigest: setDigest('history-checkpoint-witness-key-fingerprints', fingerprintValues),
      declaredPrincipalSetDigest: setDigest('history-checkpoint-witness-declared-principals', principalValues),
      attestationSetDigest: setDigest('history-checkpoint-witness-attestations', seats.map(seat => seat.attestationDigest)),
...
function normalizeAnchorPolicy(input, verifiedAt) {
  const policy = clone(input);
  exactKeys(policy, [
    'schema', 'policyId', 'anchorId', 'anchorEpoch', 'issuedAt', 'expiresAt', 'status', 'audience', 'scope',
    'authorityOrigin', 'requiredSignatures', 'maxAuthorizationAgeSeconds', 'keys', 'policyDigest'
  ...
  const issuedAt = timestamp(policy.issuedAt, 'checkpoint anchor policy issuedAt');
  const expiresAt = timestamp(policy.expiresAt, 'checkpoint anchor policy expiresAt');
  if (Date.parse(issuedAt) > Date.parse(verifiedAt) || Date.parse(expiresAt) <= Date.parse(verifiedAt) || Date.parse(expiresAt) <= Date.parse(issuedAt)) {
    throw new Error('checkpoint anchor policy is expired or has an invalid validity window');
  }
  ...
  const keys = normalizeSeats(policy.keys, {
    label: 'checkpoint anchor policy keys', keyIdField: 'keyId', principalField: 'stewardDigest', kindField: 'stewardKind', scope: ANCHOR_SCOPE
  });
...
function verifyAnchorAuthorization(input, policy, commitment, expectedAnchorDigest, witnessVerifiedAt, verifiedAt, index) {
  ...
  exactKeys(item, [
    'schema', 'authorizationId', 'keyId', 'stewardDigest', 'stewardKind', 'verdict', 'scope', 'policyDigest',
    'expectedAnchorDigest', 'checkpointDigest', 'witnessDigest', 'witnessPolicyDigest',
    'witnessKeyFingerprintSetDigest', 'witnessDeclaredPrincipalSetDigest',
  ...
  const bindings = {
    ...
    checkpointDigest: commitment.checkpointRef.sha256,
    witnessDigest: commitment.witnessRef.sha256,
  ...
  const issuedMs = Date.parse(issuedAt); const expiresMs = Date.parse(expiresAt); const verifiedMs = Date.parse(verifiedAt);
  if (issuedMs < Date.parse(policy.issuedAt) || issuedMs < Date.parse(witnessVerifiedAt) || issuedMs > verifiedMs || expiresMs <= issuedMs || expiresMs <= verifiedMs ||
      expiresMs > Date.parse(policy.expiresAt) || verifiedMs - issuedMs > policy.maxAuthorizationAgeSeconds * 1000 ||
      expiresMs - issuedMs > policy.maxAuthorizationAgeSeconds * 1000) {
    throw new Error(label + ' is expired or exceeds the anchor authorization age window');
  }
  if (item.signatureAlgorithm !== 'Ed25519') throw new Error(label + ' signatureAlgorithm must be Ed25519');
  let verified = false;
  ...
  if (!verified) throw new Error(label + ' signature verification failed');
  return {
    authorizationId, authorizationDigest: sha256(item), keyId,
    fingerprint: seat.fingerprint, principalDigest: seat.principalDigest, kind: seat.kind, issuedAt, expiresAt
  };
}
...
function buildAnchoredCheckpoint(input) {
  ...
  exactKeys(input, [
    'receiptId', 'verifiedAt', 'expectedAnchorDigest', 'witnessInput', 'witnessReceipt', 'anchorPolicy', 'policyAuthorizations'
  ], 'anchored checkpoint input');
  ...
  if (Date.parse(policy.issuedAt) < Date.parse(witnessDetailed.receipt.verifiedAt)) {
    throw new Error('checkpoint anchor policy cannot predate exact witness verification');
  }
  ...
  if (!Array.isArray(input.policyAuthorizations) || input.policyAuthorizations.length < policy.requiredSignatures || input.policyAuthorizations.length > MAX_SEATS) {
    throw new Error('checkpoint anchor authorizations must cover the threshold and contain at most ten entries');
  }
  const seats = input.policyAuthorizations.map((entry, index) => verifyAnchorAuthorization(
    entry, policy, commitment, expected, witnessDetailed.receipt.verifiedAt, verifiedAt, index
  ));
  const ids = new Set(); const keys = new Set(); const fingerprints = new Set(); const principals = new Set();
  seats.forEach(seat => {
    if (ids.has(seat.authorizationId)) throw new Error('checkpoint anchor authorizationId must be unique');
    if (keys.has(seat.keyId) || fingerprints.has(seat.fingerprint) || principals.has(seat.principalDigest)) {
      throw new Error('checkpoint anchor authorizations must occupy unique key and declared-principal seats');
    }
    ids.add(seat.authorizationId); keys.add(seat.keyId); fingerprints.add(seat.fingerprint); principals.add(seat.principalDigest);
  });
  const anchorFingerprints = seats.map(seat => seat.fingerprint);
  const anchorPrincipals = seats.map(seat => seat.principalDigest);
  ...
  const receipt = {
    ...
    receiptId,
    verifiedAt,
    checkpointRef: clone(commitment.checkpointRef),
    witnessRef: clone(commitment.witnessRef),
    witnessPolicyRef: clone(commitment.witnessPolicyRef),
    anchorPolicyRef: clone(commitment.anchorPolicyRef),
    ...
    separationEvidence: {
      witnessVerifiedSeats: witnessDetailed.seats.length,
      anchorVerifiedSeats: seats.length,
      witnessKeyFingerprintSetDigest: setDigest('history-checkpoint-witness-key-fingerprints', witnessDetailed.fingerprints),
      anchorKeyFingerprintSetDigest: setDigest('history-checkpoint-anchor-key-fingerprints', anchorFingerprints),
      witnessDeclaredPrincipalSetDigest: setDigest('history-checkpoint-witness-declared-principals', witnessDetailed.principals),
      anchorDeclaredPrincipalSetDigest: setDigest('history-checkpoint-anchor-declared-principals', anchorPrincipals),
      anchorAuthorizationSetDigest: setDigest('history-checkpoint-anchor-authorizations', seats.map(seat => seat.authorizationDigest)),
...
function anchoredAuditTruth(upstream) {
  return Object.assign(anchoredTruth(), {
    ...
    upstreamV21AuditComposedUnchanged: true,
    classificationRelativeToPresentedAnchoredCheckpointOnly: true,
    currentLedgerValidatedByV2Reload: upstream.truth.currentLedgerValidatedByV2Reload,
    everyCurrentPersistedPackageExactRebuilt: upstream.truth.everyCurrentPersistedPackageExactRebuilt,
    strictRollbackObserved: upstream.truth.strictRollbackObserved,
    replacementOrForkObserved: upstream.truth.replacementOrForkObserved,
    autonomousActionCount: 0
  });
}
function buildAnchoredAudit(input) {
  bound(input, MAX_INPUT_CANONICAL_BYTES, 'anchored audit input');
  exactKeys(input, ['auditId', 'auditedAt', 'anchoredInput', 'anchoredReceipt', 'current'], 'anchored audit input');
  ...
  exactKeys(input.current, ['serviceOptions', 'records'], 'anchored audit current ledger input');
  const upstream = History.auditCheckpoint({
    auditId: auditId + ':v2.1',
    auditedAt,
    checkpoint: clone(input.anchoredInput.witnessInput.checkpoint),
    current: clone(input.current)
  });
  const receipt = {
    schema: ANCHORED_AUDIT_SCHEMA,
    version: VERSION,
    status: STATUS,
    auditId,
    auditedAt,
    ...
    current: clone(upstream.current),
    comparison: clone(upstream.comparison),
    decision: clone(upstream.decision),
    state: 'ANCHORED_CHECKPOINT_EXACT_REBUILT_AND_COMPARED_READ_ONLY_AUTHORITY_AND_RETENTION_NOT_PROVEN',
    truth: anchoredAuditTruth(upstream),
    auditDigest: null
  };
...
module.exports = {
  ...
  WITNESS_ATTESTATION_SCHEMA,
  WITNESS_SCHEMA,
  ANCHOR_POLICY_SCHEMA,
  ANCHOR_AUTHORIZATION_SCHEMA,
  ANCHOR_COMMITMENT_SCHEMA,
  ANCHORED_CHECKPOINT_SCHEMA,
  ANCHORED_AUDIT_SCHEMA,
  VERSION,
  STATUS,
  WITNESS_AUDIENCE,
  WITNESS_SCOPE,
  ANCHOR_AUDIENCE,
  ANCHOR_SCOPE,
  POLICY_AUTHORITY_ORIGIN,
  EXPECTED_ANCHOR_ORIGIN,
  MAX_SEATS,
  ...
  sha256,
  witnessPolicyDigest,
  witnessSigningPayload,
  anchorPolicyDigest,
  anchorCommitment,
  anchorDigest,
  anchorAuthorizationPayload,
  buildWitness,
  verifyWitness,
  buildAnchoredCheckpoint,
