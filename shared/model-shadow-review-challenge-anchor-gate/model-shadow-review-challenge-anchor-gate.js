...
const ANCHOR_POLICY_SCHEMA = 'axm.model-shadow-review-challenge-anchor-policy/v1';
const AUTHORIZATION_SCHEMA = 'axm.model-shadow-review-challenge-anchor-authorization/v1';
const RECEIPT_SCHEMA = 'axm.model-shadow-review-challenge-anchored-witness/v1';
const AUDIT_SCHEMA = 'axm.model-shadow-review-challenge-anchored-continuity/v1';
const VERSION = '0.5.0';
const STATUS = 'TEST';
const AUDIENCE = 'MODEL_SHADOW_REVIEW_CHALLENGE_WITNESS_AUTHORIZATION';
const SCOPE = 'AUTHORIZE_EXACT_MODEL_SHADOW_REVIEW_CHALLENGE_WITNESS';
const AUTHORITY_ORIGIN = 'CALLER_PRESENTED_PIN_UNAUTHENTICATED';
const NEXT_GATE = 'HOST_AUTHENTICATED_ANCHOR_PIN_AND_EXTERNALLY_RETAINED_RECEIPT_OR_PROTECTED_MONOTONIC_STORE';
...
function normalizeAnchorPolicy(policyInput, verifiedAt) {
  ...
  ], 'challenge witness anchor policy');
  if (policy.schema !== ANCHOR_POLICY_SCHEMA) throw new Error('challenge witness anchor policy schema mismatch');
  const anchorId = exactText(policy.anchorId, 'challenge witness anchor id', 180);
  const anchorEpoch = positiveInteger(policy.anchorEpoch, 'challenge witness anchor epoch', 1, Number.MAX_SAFE_INTEGER);
  const issuedAt = timestamp(policy.issuedAt, 'challenge witness anchor issuedAt');
  const expiresAt = timestamp(policy.expiresAt, 'challenge witness anchor expiresAt');
  if (Date.parse(issuedAt) > Date.parse(verifiedAt)) throw new Error('challenge witness anchor cannot be issued after verification');
  if (Date.parse(expiresAt) <= Date.parse(verifiedAt) || Date.parse(expiresAt) <= Date.parse(issuedAt)) {
    throw new Error('challenge witness anchor is expired or has an invalid validity window');
  }
  if (policy.status !== STATUS) throw new Error('challenge witness anchor status mismatch');
  if (policy.audience !== AUDIENCE) throw new Error('challenge witness anchor audience mismatch');
  if (policy.scope !== SCOPE) throw new Error('challenge witness anchor scope mismatch');
  if (policy.authorityOrigin !== AUTHORITY_ORIGIN) {
    throw new Error('challenge witness anchor must admit its caller-presented unauthenticated origin');
  }
  const requiredSignatures = positiveInteger(policy.requiredSignatures, 'challenge witness anchor requiredSignatures', 1, 10);
  const maxAuthorizationAgeSeconds = positiveInteger(
    policy.maxAuthorizationAgeSeconds,
    'challenge witness anchor maxAuthorizationAgeSeconds',
    1,
    86400
  );
  if (!Array.isArray(policy.keys) || policy.keys.length < requiredSignatures || policy.keys.length > 10) {
    throw new Error('challenge witness anchor keys must cover the signature threshold and contain at most ten entries');
  }
  const keyIds = new Set();
  const stewardDigests = new Set();
  const fingerprints = new Set();
  const keys = policy.keys.map((entry, index) => {
    const label = 'challenge witness anchor key[' + index + ']';
    ...
    if (keyIds.has(anchorKeyId)) throw new Error('challenge witness anchor key id must be unique');
    ...
    if (stewardDigests.has(stewardDigest)) throw new Error('challenge witness anchor stewardDigest must be unique');
    ...
    if (fingerprints.has(fingerprint)) throw new Error('challenge witness anchor public key fingerprint must be unique');
    fingerprints.add(fingerprint);
    return { anchorKeyId, stewardDigest, stewardKind, key, fingerprint };
  });
  const expectedDigest = anchorPolicyDigest(policy);
  if (digest(policy.anchorDigest, 'challenge witness anchor digest') !== expectedDigest) {
    throw new Error('challenge witness anchor digest mismatch');
  }
  return {
    anchorId,
    anchorEpoch,
...
function verifyAuthorization(input, anchor, expectedAnchorDigest, witness, verifiedAt, index) {
  ...
  const label = 'challenge witness anchor authorization[' + index + ']';
  exactKeys(authorization, [
    ...
    'ledgerManifestDigest', 'entriesDigest', 'issuedAt', 'expiresAt',
  ...
  if (digest(authorization.stewardDigest, label + '.stewardDigest') !== key.stewardDigest) throw new Error(label + ' stewardDigest mismatch');
  if (authorization.stewardKind !== key.stewardKind) throw new Error(label + ' stewardKind mismatch');
  ...
  if (authorization.ledgerManifestDigest !== witness.ledgerRef.sha256) throw new Error(label + ' ledger manifest digest mismatch');
  ...
  const maxAgeMs = anchor.maxAuthorizationAgeSeconds * 1000;
  ...
  return {
    authorizationId,
    authorizationIdDigest: sha256('challenge-witness-anchor-authorization-id:' + authorizationId),
    authorizationDigest: sha256(authorization),
    anchorKeyIdDigest: sha256('challenge-witness-anchor-key-id:' + key.anchorKeyId),
...
function anchoredWitnessTruth() {
  return {
    ...
    jointAnchorAndPinSubstitutionStillPossible: true,
...
function buildAnchoredWitness(input) {
  ...
  ], 'anchored witness input');
  const receiptId = exactText(input.receiptId, 'anchored witness receipt id', 180);
  const verifiedAt = timestamp(input.verifiedAt, 'anchored witness verifiedAt');
  const expectedAnchorDigest = digest(input.expectedAnchorDigest, 'expected challenge witness anchor digest');
  ...
  if (Date.parse(verifiedAt) < Date.parse(witness.verifiedAt)) {
    throw new Error('anchored witness verification cannot predate checkpoint witness verification');
  }
  const anchor = normalizeAnchorPolicy(input.anchorPolicy, verifiedAt);
  if (expectedAnchorDigest !== anchor.anchorDigest) {
    throw new Error('caller-presented expected anchor digest does not match the exact anchor policy');
  }
  if (!Array.isArray(input.policyAuthorizations) ||
      input.policyAuthorizations.length < anchor.requiredSignatures ||
      input.policyAuthorizations.length > 10) {
    throw new Error('challenge witness anchor authorizations must cover the anchor threshold and contain at most ten entries');
  }
  ...
  for (const authorization of authorizations) {
    if (ids.has(authorization.authorizationId)) throw new Error('challenge witness anchor authorizationId must be unique');
    if (stewards.has(authorization.stewardDigest)) throw new Error('challenge witness anchor stewardDigest must fill at most one seat');
    if (fingerprints.has(authorization.keyFingerprint)) throw new Error('challenge witness anchor public key must fill at most one seat');
  ...
  const receipt = {
    ...
    ledgerRef: clone(witness.ledgerRef),
    entriesDigest: witness.entriesDigest,
    state: 'WITNESS_CHAIN_VERIFIED_TO_CALLER_PRESENTED_ANCHOR_PIN_AUTHORITY_NOT_AUTHENTICATED',
...
function verifyAnchoredWitness(input, receipt) {
  ...
  let rebuilt = null;
  try {
    if (!receipt || receipt.schema !== RECEIPT_SCHEMA) throw new Error('anchored witness receipt schema mismatch');
    rebuilt = buildAnchoredWitness(clone(input));
  } catch (error) {
    errors.push(error.message);
  }
  if (rebuilt && stableStringify(rebuilt) !== stableStringify(receipt)) {
    errors.push('anchored witness receipt content or digest mismatch');
  }
...
function anchoredAuditTruth(witnessedAudit) {
  return {
    ...
    comparisonBoundToExactLedgerIdentity: witnessedAudit.truth.comparisonBoundToExactLedgerIdentity,
...
function buildAnchoredAudit(input) {
  ...
  ], 'anchored continuity audit input');
  const auditId = exactText(input.auditId, 'anchored continuity audit id', 180);
  const checkedAt = timestamp(input.checkedAt, 'anchored continuity audit checkedAt');
  ...
  if (Date.parse(checkedAt) < Date.parse(anchored.verifiedAt)) {
    throw new Error('anchored continuity audit cannot predate anchored witness verification');
  }
  ...
  const audit = {
    ...
    state: 'ANCHORED_WITNESS_CHAIN_VERIFIED_AND_COMPARED_AUTHORITY_NOT_AUTHENTICATED',
    nextGate: NEXT_GATE,
...
function verifyAnchoredAudit(input, receipt) {
  ...
  let rebuilt = null;
  try {
    if (!receipt || receipt.schema !== AUDIT_SCHEMA) throw new Error('anchored continuity audit schema mismatch');
    rebuilt = buildAnchoredAudit(clone(input));
  } catch (error) {
    errors.push(error.message);
  }
  if (rebuilt && stableStringify(rebuilt) !== stableStringify(receipt)) {
    errors.push('anchored continuity audit content or digest mismatch');
  }
  return { pass: errors.length === 0, errors, rebuilt };
}
