...
(async () => {
  ...
  assert.equal(new Set(first.checks.map(item => item.checkId)).size, 10);
  assert(pulse.status().recentReceipts.filter(item => item.goalId === first.goalId).length === 10);
  const held = await bridge.onBeat({ beatId: 'beat-2', sequence: 2, kind: 'SCHEDULED' });
  assert.equal(held.reason, 'ten-per-hour-cap');
  clock += 3600000;
  ...
  assert.equal(bridge.status().maxChecksPerHour, 10);
  ...
  console.log('PASS heartbeat verification bridge / ten hourly pulse leases / findings only');
